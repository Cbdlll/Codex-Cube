...
export function CodexFormFields({
  ...
  // 从拉取结果选择模型时，优先自动带上上下文窗口（供应商返回时）。
  const handleSelectFetchedCatalogModel = useCallback(
    (index: number, id: string) => {
      setCatalogRows((current) =>
        current.map((row, i) => {
          if (i !== index) return row;
          const fetched = fetchedModels.find((model) => model.id === id);
          const patch: Partial<CodexCatalogModel> = {
            model: id,
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-xs leading-relaxed text-muted-foreground">
                    {t("codexConfig.modelMappingHint", {
                      defaultValue:
                        "选择模型角色后，Codex Cube 会自动生成 Codex 兼容路由；菜单显示名可以填 DeepSeek、Kimi 等品牌模型，实际请求模型按右侧填写内容发送。",
                    })}
                  </p>
                </div>
                        ...
                        <div className="flex gap-1">
                          ...
                          {fetchedModels.length > 0 && (
                            <ModelDropdown
                              ...
                              onSelect={(id) =>
                                handleSelectFetchedCatalogModel(index, id)
                              }
                            />
                          )}
                        </div>
