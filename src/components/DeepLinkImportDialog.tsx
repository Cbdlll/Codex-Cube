...
export function DeepLinkImportDialog() {
  ...
  useEffect(() => {
    ...
    const unlistenImport = listen<DeepLinkImportRequest>(
      ...
      async (event) => {
        const isProvider =
          event.payload.resource === "provider" || !event.payload.resource;
        if (!isProvider || event.payload.app !== "codex") {
          toast.error(
            t("deeplink.codexOnlyError", {
              defaultValue: "Codex Cube 仅支持导入 Codex 供应商配置",
            }),
          );
          return;
        }
