...
import { Loader2, Save, FolderSearch, Database } from "lucide-react";
...
export function SettingsPage({
  ...
  // 通用设置即时保存（无需手动点击）
  // 使用 autoSaveSettings 避免误触发系统 API（例如开机自启）
  ...
  return (
    <div className="mx-auto flex h-full w-full max-w-5xl flex-col overflow-hidden px-6">
      {isBusy ? (
        ...
        >
          <div className="flex min-h-0 flex-1 flex-col overflow-hidden rounded-xl border border-border bg-card shadow-sm">
            <div className="flex-shrink-0 border-b border-border p-4">
              <TabsList className="grid w-full grid-cols-6 rounded-lg bg-muted p-1">
                <TabsTrigger
                  value="general"
                  className="min-w-0 px-2 data-[state=active]:bg-card data-[state=active]:text-foreground data-[state=active]:shadow-sm"
                >
