...
const GITHUB_REPO_URL = "https://github.com/Cbdlll/Codex-Cube";
...
export function AboutSection({ isPortable }: AboutSectionProps) {
  ...
  const loadAllToolVersions = useCallback(
    async (options?: { force?: boolean }) => {
      ...
      // 命中新鲜缓存：切回「关于」Tab 触发的重挂直接复用上次结果，跳过 Codex
      // 版本与 latest 版本探测。手动「刷新」传 force 绕过缓存强制重查。
      if (
        !force &&
        toolVersionsCache &&
      ...
      try {
        ...
        // 每个配置工具独立刷新，避免一个探测失败拖累其余结果。refreshTool-
  ...
  useEffect(() => {
    ...
    // 工具检查完成之后——图标下方的版本徽标不应等待 Codex 探测才显示。
  ...
  const handleOpenReleaseNotes = useCallback(async () => {
    try {
      ...
      await settingsApi.openExternal(
        `${GITHUB_REPO_URL}/releases/tag/${displayVersion}`,
      );
    } catch (error) {
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-8">
            <div className="flex flex-col items-center gap-2">
              <div className="flex items-center gap-2">
                <button
                  type="button"
                  onClick={() => void settingsApi.openExternal(GITHUB_REPO_URL)}
                  className="bg-gradient-to-r from-indigo-500 via-violet-500 to-cyan-400 bg-clip-text text-lg font-bold text-transparent"
          ...
          <div className="flex items-center gap-2">
            <Button
              ...
              variant="outline"
              size="sm"
              onClick={() => settingsApi.openExternal(GITHUB_REPO_URL)}
