...
export function ProxyPanel({
  ...
  const [listenPort, setListenPort] = useState("15921");
              ...
              <div className="rounded-xl border-2 border-primary/20 bg-primary/5 p-4 space-y-3">
                ...
                <div className="grid gap-2">
                  <div className="flex items-center justify-between rounded-md border border-primary/20 bg-background/60 px-3 py-2">
                    ...
                    <Switch
                      checked={takeoverStatus?.codex ?? false}
                      onCheckedChange={(checked) =>
                        handleTakeoverChange("codex", checked)
                      }
                ...
                {status.active_targets?.some(
                  ...
                  <div className="grid gap-2 sm:grid-cols-2">
                    {status.active_targets
                      .filter((target) => target.app_type === "codex")
                      .map((target) => (
                        <div
                          key={target.app_type}
