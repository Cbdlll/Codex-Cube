...
import {
  canonicalCodexWireApiFromWireApi,
  codexApiFormatFromWireApi,
  codexWireApiFromApiFormat,
...
describe("Codex wire API helpers", () => {
  ...
  it("maps form apiFormat to canonical wire api", () => {
    expect(codexWireApiFromApiFormat("openai_chat")).toBe("chat");
    expect(codexWireApiFromApiFormat("anthropic")).toBe("anthropic");
    expect(codexWireApiFromApiFormat("openai_responses")).toBe("responses");
    expect(codexWireApiFromApiFormat(undefined)).toBe("responses");
  });

  it("normalizes every legal alias to canonical wire api", () => {
    expect(canonicalCodexWireApiFromWireApi("openai_responses")).toBe(
      "responses",
    );
    expect(canonicalCodexWireApiFromWireApi("openai-responses")).toBe(
      "responses",
    );
    expect(canonicalCodexWireApiFromWireApi("chat_completions")).toBe("chat");
    expect(canonicalCodexWireApiFromWireApi("chat-completions")).toBe("chat");
    expect(canonicalCodexWireApiFromWireApi("openai_chat")).toBe("chat");
    expect(canonicalCodexWireApiFromWireApi("openai-chat")).toBe("chat");
    expect(canonicalCodexWireApiFromWireApi("openai_chat_completions")).toBe(
      "chat",
    );
    expect(canonicalCodexWireApiFromWireApi("anthropic_messages")).toBe(
      "anthropic",
    );
    expect(canonicalCodexWireApiFromWireApi("anthropic-messages")).toBe(
      "anthropic",
    );
    expect(canonicalCodexWireApiFromWireApi("claude")).toBe("anthropic");
    expect(canonicalCodexWireApiFromWireApi("messages")).toBe("anthropic");
    expect(canonicalCodexWireApiFromWireApi("bogus")).toBe("responses");
    expect(canonicalCodexWireApiFromWireApi(undefined)).toBe("responses");
  });

  it("resolves canonical wire api from meta apiFormat (chat)", () => {
    expect(resolveCodexWireApi({ metaApiFormat: "openai_chat" })).toBe("chat");
    expect(resolveCodexWireApi({ metaApiFormat: "anthropic" })).toBe(
      "anthropic",
    );
    expect(resolveCodexWireApi({ metaApiFormat: "openai_responses" })).toBe(
      "responses",
    );
  });

  it("resolves canonical wire api from settings apiFormat / api_format", () => {
    expect(resolveCodexWireApi({ settingsApiFormat: "chat" })).toBe("chat");
    expect(resolveCodexWireApi({ settingsApiFormat: "chat_completions" })).toBe(
      "chat",
    );
    expect(resolveCodexWireApi({ settingsApiFormatSnake: "anthropic" })).toBe(
      "anthropic",
    );
    expect(resolveCodexWireApi({ settingsApiFormatSnake: "openai_chat" })).toBe(
      "chat",
    );
  });

  it("resolves canonical wire api from TOML config wire_api aliases", () => {
    expect(resolveCodexWireApi({ configText: 'wire_api = "chat"' })).toBe(
      "chat",
    );
    expect(
      resolveCodexWireApi({ configText: 'wire_api = "openai_responses"' }),
    ).toBe("responses");
    expect(
      resolveCodexWireApi({ configText: 'wire_api = "anthropic_messages"' }),
    ).toBe("anthropic");
    expect(
      resolveCodexWireApi({
        configText:
  ...
  it("resolves canonical wire api from full endpoint URL", () => {
    expect(
      resolveCodexWireApi({
    ...
    expect(
      resolveCodexWireApi({ baseUrl: "https://api.example.com/v1/messages" }),
    ).toBe("anthropic");
    expect(
      resolveCodexWireApi({ baseUrl: "https://api.example.com/v1/responses" }),
    ).toBe("responses");
    expect(resolveCodexWireApi({ baseUrl: "https://api.example.com/v1" })).toBe(
      "responses",
    );
  });

  it("prefers explicit format over TOML wire_api and URL, and defaults to responses", () => {
    expect(
      resolveCodexWireApi({
        metaApiFormat: "anthropic",
        configText: 'wire_api = "chat"',
        baseUrl: "https://api.example.com/chat/completions",
      }),
    ).toBe("anthropic");
    expect(
    ...
    expect(resolveCodexWireApi({})).toBe("responses");
    expect(resolveCodexWireApi({ baseUrl: "" })).toBe("responses");
  });
});
