...
/** 规范 wire_api：写入 Codex config / 传给后端的唯一合法集合。 */
export type CodexCanonicalWireApi = "responses" | "chat" | "anthropic";

/** 把内部 apiFormat 别名映射到规范 wire_api（responses | chat | anthropic）。 */
export const codexWireApiFromApiFormat = (
  format: CodexApiFormat | undefined | null,
): CodexCanonicalWireApi => {
  if (format === "openai_chat") return "chat";
  if (format === "anthropic") return "anthropic";
  return "responses";
};

/** 把任意合法 wire_api / apiFormat 别名归一化为规范 wire_api（responses | chat | anthropic）。 */
export const canonicalCodexWireApiFromWireApi = (
  value: string | undefined | null,
): CodexCanonicalWireApi =>
  codexWireApiFromApiFormat(codexApiFormatFromWireApi(value));

/** 从完整 endpoint URL 推断协议：chat/completions => chat，messages => anthropic，responses => responses。 */
function codexWireApiFromEndpointUrl(
  baseUrl: string | undefined | null,
): CodexCanonicalWireApi | undefined {
...
export interface ResolveCodexWireApiInput {
  /** provider.meta.apiFormat */
  metaApiFormat?: CodexApiFormat | string | null;
  /** settingsConfig.apiFormat */
  settingsApiFormat?: unknown;
  /** settingsConfig.api_format */
  settingsApiFormatSnake?: unknown;
  /** settingsConfig.config（Codex TOML） */
  configText?: string | null;
  /** 完整 endpoint URL（base_url / baseURL / TOML base_url） */
  baseUrl?: string | null;
}

/**
 * 解析 Codex 供应商的规范 wire_api（responses | chat | anthropic）。
 * 覆盖 meta.apiFormat / settings apiFormat / api_format / TOML config wire_api /
 * 完整 endpoint URL，全部复用既有别名归一化函数；未知值默认 responses。
 * 判定顺序与后端 src-tauri/proxy/providers/codex.rs 保持一致。
 */
export const resolveCodexWireApi = (
  input: ResolveCodexWireApiInput,
): CodexCanonicalWireApi => {
  const explicit =
    input.metaApiFormat ??
    input.settingsApiFormatSnake ??
    input.settingsApiFormat;
  const fromExplicit = codexApiFormatFromWireApi(
    typeof explicit === "string" ? explicit : null,
  );
  if (fromExplicit) return codexWireApiFromApiFormat(fromExplicit);
  const fromToml = codexApiFormatFromWireApi(
    extractCodexWireApi(input.configText),
  );
  if (fromToml) return codexWireApiFromApiFormat(fromToml);
  return codexWireApiFromEndpointUrl(input.baseUrl) ?? "responses";
};
