...
export const skillsApi = {
  ...
  /** 获取技能列表（兼容旧 API） */
  async getAll(app: AppId = "codex"): Promise<Skill[]> {
    return await invoke("get_skills_for_app", { app });
  },

  /** 安装技能（兼容旧 API） */
  async install(directory: string, app: AppId = "codex"): Promise<boolean> {
