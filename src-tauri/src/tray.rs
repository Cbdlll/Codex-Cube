...
pub const TRAY_ID: &str = "codex-cube";
...
pub fn handle_tray_menu_event(app: &tauri::AppHandle, event_id: &str) {
    ...
    match event_id {
        ...
        "open_website" => {
            if let Err(e) = app
                .opener()
                .open_url("https://github.com/Cbdlll/Codex-Cube", None::<String>)
            {
...
mod tests {
    ...
    use crate::services::subscription::{
        ...
        TIER_SEVEN_DAY, TIER_SEVEN_DAY_OPUS, TIER_SEVEN_DAY_SONNET, TIER_THIRTY_DAY,
        TIER_WEEKLY_LIMIT,
    };

    #[test]
    fn tray_id_is_unique_to_app() {
        assert_eq!(TRAY_ID, "codex-cube");
    ...
    #[test]
    fn tray_sections_only_include_codex() {
    ...
    #[test]
    fn codex_summary_uses_h_and_w_labels() {
    ...
    fn failure_quota_returns_none() {
        let quota = make_quota("codex", false, vec![tier("five_hour", 50.0)]);
        assert!(format_subscription_summary(&quota).is_none());
    }

    #[test]
    fn unknown_tiers_return_none() {
        let quota = make_quota("codex", true, vec![tier("one_hour", 80.0)]);
        assert!(format_subscription_summary(&quota).is_none());
    }

    #[test]
    fn unknown_tiers_return_none_keeps_codex_semantics() {
        // 完全没有已知 tier 的退化响应 → None。
        let quota = make_quota("codex", true, vec![tier("some_future_tier", 80.0)]);
        assert!(format_subscription_summary(&quota).is_none());
    }
    ...
    #[test]
    fn script_summary_official_subscription_uses_h_and_w_labels() {
        let r = usage_result(
            true,
            vec![
                usage_data(Some(TIER_FIVE_HOUR), 12.0),
