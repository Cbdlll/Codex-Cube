...
/// 应用设置结构
///
/// 存储设备级别设置，保存在本地 `~/.codex-cube/settings.json`，不随数据库同步。
/// 这确保了云同步场景下多设备可以独立运作。
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
...
mod tests {
    ...
    #[test]
    fn visible_apps_old_settings_default_codex_visible() {
        let visible: VisibleApps = serde_json::from_value(serde_json::json!({
            "codex": true
        }))
        .expect("visible apps");

        // codex 缺省时按 default_true 显示
        assert!(visible.is_visible(&AppType::Codex));
    }

    #[test]
    fn visible_apps_accepts_codex_flag() {
        let visible: VisibleApps = serde_json::from_value(serde_json::json!({
            "codex": false
        }))
        .expect("visible apps");
