...
pub(crate) fn has_matching_proxy_usage_log(
    ...
    let allow_missing_cache_creation = key.app_type == "codex" && key.cache_creation_tokens == 0;
...
mod tests {
    ...
    #[test]
    fn test_backfill_deducts_cache_read_for_codex_total_rows() -> Result<(), AppError> {
        // 回归：回填侧的 cache-inclusive 判定收敛到
        // sql_helpers::is_cache_inclusive_app 后按 fresh 计价。
        let db = Database::memory()?;
        {
            let conn = lock_conn!(db.conn);
            insert_usage_log(
                &conn,
                "codex-total-backfill",
                "codex",
                "_codex_session",
                "gpt-5.5",
                "codex_session",
            ...
            conn.execute(
                "UPDATE proxy_request_logs
                 SET input_token_semantics = ?1
                 WHERE request_id = 'codex-total-backfill'",
                [INPUT_TOKEN_SEMANTICS_TOTAL],
            )?;
        }
        ...
        let (input_cost, output_cost, cache_read_cost, total_cost): (
            ...
            "SELECT input_cost_usd, output_cost_usd, cache_read_cost_usd, total_cost_usd
             FROM proxy_request_logs WHERE request_id = 'codex-total-backfill'",
            [],
            |row| Ok((row.get(0)?, row.get(1)?, row.get(2)?, row.get(3)?)),
        )?;
        // gpt-5.5 定价 5/30/0.50：input = (700-250)×5/1M，output = 100×30/1M，
        // cache_read = 250×0.5/1M
        assert_eq!(input_cost, "0.002250");
        assert_eq!(output_cost, "0.003000");
        assert_eq!(cache_read_cost, "0.000125");
        assert_eq!(total_cost, "0.005375");
        Ok(())
    }
    ...
    fn test_effective_usage_dedup_prefers_proxy_for_session_sources() -> Result<(), AppError> {
        ...
        // codex-proxy 计入 fresh input = 100-10=90（LEGACY 语义只扣 cache_read）；
        // codex-proxy-2 = 200-30=170；codex-session-only = 50。90 + 170 + 50 = 310。
        assert_eq!(summary.total_input_tokens, 310);
        assert_eq!(summary.total_output_tokens, 65);
        assert_eq!(summary.total_cache_read_tokens, 40);
        assert_eq!(summary.total_cache_creation_tokens, 7);
        // real_total = fresh_input(310) + output(65) + cache_create(7) + cache_read(40) = 422
        assert_eq!(summary.real_total_tokens, 422);
        // hit_rate = 40 / (310 + 7 + 40) = 40 / 357
        let expected_hit_rate = 40.0_f64 / 357.0_f64;
