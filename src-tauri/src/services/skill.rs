...
pub enum SkillStorageLocation {
    /// Codex-Cube 管理目录 (~/.codex-cube/skills/)
    #[default]
    CodexCube,
...
impl SkillService {
    ...
    /// 获取 SSOT 目录（根据设置返回 ~/.codex-cube/skills/ 或 ~/.agents/skills/）
    pub fn get_ssot_dir() -> Result<PathBuf> {
        let location = crate::settings::get_skill_storage_location();
        let dir = match location {
            SkillStorageLocation::CodexCube => get_app_config_dir().join("skills"),
    ...
    /// 获取 Skill 卸载备份目录（~/.codex-cube/skill-backups/）
    ...
    pub fn get_app_skills_dir(app: &AppType) -> Result<PathBuf> {
        ...
        // 默认路径：回退到用户主目录下的标准位置。
        // 必须走 get_home_dir()（可被 CODEX_CUBE_TEST_HOME 覆盖）：Windows 上 dirs::home_dir()
    ...
    /// 扫描未管理的 Skills
    ///
    /// 扫描各应用目录，找出未被 Codex-Cube 管理的 Skills
    pub fn scan_unmanaged(db: &Arc<Database>) -> Result<Vec<UnmanagedSkill>> {
        ...
        if let Ok(ssot_dir) = Self::get_ssot_dir() {
            scan_sources.push((ssot_dir, "codex-cube".to_string()));
        }
    ...
    /// 从应用目录导入 Skills
    ///
    /// 将未管理的 Skills 导入到 Codex-Cube 统一管理
    pub fn import_from_apps(
        ...
        search_sources.push((ssot_dir.clone(), "codex-cube".to_string()));
    ...
    fn resolve_uninstall_backup_source(skill: &InstalledSkill) -> Result<Option<PathBuf>> {
        // 返回值会被整目录复制进 ~/.codex-cube/skill-backups/ 并由 get_skill_backups
...
mod tests {
    ...
    fn validate_repo_ref_accepts_real_world_coordinates() {
        ...
        for branch in [
            ...
            assert!(
                SkillService::validate_repo_ref("example", "repo", branch).is_ok(),
    ...
    fn validate_repo_ref_accepts_the_empty_branch_sentinel() {
        ...
        assert!(
            SkillService::validate_repo_ref("example", "repo", "").is_ok(),
    ...
    /// CODEX_CUBE_TEST_HOME 隔离守卫（serial 测试间互斥由 #[serial] 保证，
    /// 守卫只负责在测试结束后恢复原值）。
    ...
    fn persist_updated_skill_metadata_uses_database_apps() {
        ...
        installed.apps = SkillApps::default();
        db.save_skill(&installed).expect("seed skill");

        // 模拟下载期间用户将 Skill 切换为 Codex 启用。待写入的 metadata
        // 仍携带下载开始时的旧 apps 快照（未启用）。
    ...
    fn restore_from_backup_rejects_traversal_directory_in_metadata() {
        ...
        // SSOT 位于 {home}/.codex-cube/skills，"../../pwned-restore" 若生效会写到 {home}/pwned-restore。
        ...
        let result = SkillService::restore_from_backup(&db, backup_id, &AppType::Codex);
    ...
    fn uninstall_rejects_traversal_directory_from_db_row() {
        ...
        // 与 import_sql_string_for_sync 的效果一致）。SSOT = {home}/.codex-cube/skills，
    ...
    #[test]
    // serial：与 backup/s3_sync/deeplink 等同样读写进程级 CODEX_CUBE_TEST_HOME 的测试互斥，
    // EnvGuard 只负责恢复不提供互斥。
    #[serial_test::serial]
    fn get_app_skills_dir_honors_test_home_override() {
        // 回归：曾直呼 dirs::home_dir() 绕过 CODEX_CUBE_TEST_HOME——Unix 上碰巧跟 $HOME
        ...
        let temp = tempdir().expect("tempdir");
        let _guard = EnvGuard(std::env::var_os("CODEX_CUBE_TEST_HOME"));
        std::env::set_var("CODEX_CUBE_TEST_HOME", temp.path());

        let dir =
            SkillService::get_app_skills_dir(&AppType::Codex).expect("resolve codex skills dir");
        assert!(
