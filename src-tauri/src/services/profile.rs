...
impl ProfileScope {
    ...
    pub const ALL: [ProfileScope; 1] = [ProfileScope::Codex];
...
mod tests {
    ...
    fn test_payload_tolerates_missing_fields() {
        // 前向兼容：旧版 payload 仍含 claude/claude-desktop 槽位，未知字段
        // 被忽略、codex 槽位缺失时落到 None（"该侧未拍过"）而不是报错，
        // 应用时对缺失槽位不做任何改动
        let back: ProfilePayload = serde_json::from_str(
            r#"{"providers":{"claude":"p1","codex":"c1"},"mcp":{"claude":["m1"]}}"#,
        )
        .unwrap();
    ...
    fn test_merge_scope_from_only_touches_scope_slots() {
        ...
        // 在 Codex 页"以当前状态更新"：只覆盖 codex 组槽位
        ...
        payload.merge_scope_from(&fresh, ProfileScope::Codex);

        assert_eq!(payload.providers.codex, Some("c2".to_string()));
        assert_eq!(payload.mcp.codex, Some(ids(&["m2"])));
    }
    ...
    fn test_scope_app_grouping() {
        ...
        assert_eq!(
            ProfileScope::for_app(&AppType::Codex),
            Some(ProfileScope::Codex)
        );
    }
