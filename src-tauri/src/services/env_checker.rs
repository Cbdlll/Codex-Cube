...
fn matches_env_keyword(name: &str, keywords: &[EnvKeyword]) -> bool {
    ...
    keywords.iter().any(
        |keyword| matches!(keyword, EnvKeyword::Prefix(prefix) if upper_name.starts_with(prefix)),
    )
}
...
mod tests {
    ...
    #[test]
    fn codex_keywords_match_only_at_the_start() {
        let keywords = get_keywords_for_app("codex");

        assert!(matches_env_keyword("OPENAI_API_KEY", &keywords));
        assert!(matches_env_keyword("OPENAI_BASE_URL", &keywords));
        assert!(!matches_env_keyword("MY_OPENAI_API_KEY", &keywords));
        assert!(!matches_env_keyword("NOT_OPENAI", &keywords));
    }
}
