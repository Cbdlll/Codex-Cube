...
impl ProxyService {
    ...
    pub async fn sync_codex_live_from_provider_while_proxy_active(
        ...
        if let Some(existing_live) = existing_live.as_ref() {
            if provider.is_aggregate() {
                // 聚合 Provider 没有自己的 config；公共配置快照可能滞后于用户在
                // Codex 里的最新设置（语言、插件、MCP 等）。以现有 Live 配置为
                // 基底，仅投影聚合路由字段，避免热切换时把这些设置重置为默认。
                if let Some(config_text) =
                    existing_live.get("config").and_then(|value| value.as_str())
                {
                    effective_settings["config"] = json!(config_text);
                }
            } else {
                ...
                Self::preserve_codex_live_user_settings(
                    &mut effective_settings,
                    existing_live,
                    provider,
                )?;
    ...
    /// Remove a runtime target only if it still points at a provider being deleted.
    pub(crate) async fn clear_active_target_if_matches(&self, app_type: &str, provider_id: &str) {
        if let Some(server) = self.server.read().await.as_ref() {
            server
                .clear_active_target_if_matches(app_type, provider_id)
                .await;
        }
    }
    ...
    async fn rollback_hot_switch_preparation(
        ...
        let live_result = if live_taken_over && matches!(app_type, AppType::Codex) {
    ...
    pub async fn start_with_takeover(&self) -> Result<ProxyServerInfo, String> {
        ...
        // 2. 仅为无凭据的旧 Provider 从同上游 Live 配置补齐 Token。
        // Provider 已保存的独立凭据始终是权威来源，不能被 auth.json 覆盖。
    ...
    pub async fn set_takeover_for_app(&self, app_type: &str, enabled: bool) -> Result<(), String> {
        ...
        if enabled {
            ...
            if restore_existing_backup_before_takeover {
                ...
                // 4) 仅为无凭据且上游匹配的当前 Provider 补齐 Live Token
    ...
    /// 为缺失凭据的旧 Provider 从 Live 配置补齐 Token
    ///
    /// Provider 已保存的凭据是 SSOT；只有 Provider 缺少凭据且 Live 与其上游
    /// 地址一致时才能导入，避免 auth.json 中上一账号的 key 串到当前 Provider。
            ...
            AppType::Codex => {
                ...
                if let Some(provider_id) = provider_id {
                        ...
                        self.db.get_provider_by_id(&provider_id, "codex")
                    {
                        ...
                        // Provider storage is the credential SSOT. `auth.json`
                        // can belong to a previously selected provider (or the
                        // official login), so takeover startup must never use it
                        // to replace an explicit key already saved on the current
                        // provider. Keep the legacy import path only for rows that
                        // genuinely have no stored credential.
                        let stored_config = provider
                            .settings_config
                            .get("config")
                            .and_then(Value::as_str);
                        if crate::codex_config::extract_codex_api_key(
                        ...
                        {
                            log::info!(
                                "跳过 Codex Live Token 回填：Provider 已有独立凭据 (provider: {provider_id})"
                            );
                            return Ok(());
                        }
                            ...
                            .filter(|s| !s.is_empty() && *s != PROXY_TOKEN_PLACEHOLDER)
                        {
                            ...
                            let provider_base_url =
                                stored_config.and_then(crate::codex_config::extract_codex_base_url);
                            let same_upstream = live_base_url
                                .as_deref()
                                .zip(provider_base_url.as_deref())
                                .is_some_and(|(live, provider)| {
                                    live.trim_end_matches('/') == provider.trim_end_matches('/')
                                });
                            if !same_upstream {
                                log::warn!(
                                    "跳过 Codex Live Token 回填：Live 上游与 Provider 上游不一致 (provider: {provider_id})"
                                );
                                return Ok(());
                            }
        ...
        match app_type {
            AppType::Codex => {
                if let Ok(Some(backup)) = self.db.get_live_backup("codex").await {
                    ...
                    // 保留用户在接管期间由 Codex Desktop 写入 Live 的偏好设置
                    // （推理档位 / personality / disable_response_storage 等），
                    // 否则每次退出/重启都会被还原成接管前状态。
                    if let Ok(live) = self.read_codex_live() {
                        config = crate::codex_config::merge_codex_live_user_settings_into_backup(
                            &config, &live,
                        );
                    }
                ...
                if let Err(e) = crate::codex_config::restore_or_clear_codex_models_cache(
                    ...
                    log::warn!("恢复 Codex models_cache 失败: {e}");
                }
            }
        }
    ...
    pub(crate) async fn restore_live_config_for_app_with_fallback_inner(
        ...
        if let Some(backup) = backup {
            ...
            if Self::live_has_proxy_placeholder_for_app(app_type, &config) {
                ...
                // Codex：把用户在被接管 Live 里的偏好设置（推理档位 /
                // personality / disable_response_storage 等）合并回备份再恢复，
                // 避免每次退出/崩溃恢复都把这些设置重置为默认。
    ...
    async fn update_live_backup_from_provider_inner(
        ...
        if matches!(app_type_enum, AppType::Codex) {
            ...
            // 热切换更新备份时，把 Live 中被接管期间保存的用户偏好（推理档位、
            // personality、desktop/plugins/MCP 等）合并进新备份，避免释放接管后
            // 重置。独立订阅的 model 重新固定为 provider 自有模型，防止聚合残留
            // 的 gpt 插槽污染备份（备份会在释放接管时写回 config.toml）。
            if !provider.is_aggregate() {
    ...
    pub(crate) async fn hot_switch_provider_inner(
        ...
        // Live config/catalog/cache and persisted current are ready at this point.
        // Commit the running proxy target in both directions so the next request
        // cannot be handled by the provider selected before this hot switch.
        ...
        // Persisted current provider and the live/runtime target are committed.
        // Rewrite stale `custom` thread models in the Codex state DB to the new
        // provider's upstream model (ordinary third-party providers only) so
        // subsequent resume and direct mode stop sending the old provider's
        // model. The post-switch config text is read so a `sqlite_home`
        // override is honored. Failure logs a warning and does not roll back
        // the successful hot switch.
        if matches!(app_type_enum, AppType::Codex) {
            ...
            if outcome.updated_rows > 0 {
                log::info!("热切换后已同步 {} 条 Codex 线程模型", outcome.updated_rows);
            }
            if !outcome.errors.is_empty() {
                log::warn!(
                    "热切换后同步 Codex 线程模型失败（不影响已成功的切换）: {}",
                    outcome.errors.join("; ")
                );
            }
        }
    ...
    /// 热切换/热同步时保留 Live 中被接管期间的用户偏好设置。
    ///
    /// `merge_codex_live_user_settings_into_backup` 会把 Live 里除接管投影字段外的
    /// 顶层键（推理档位、personality、disable_response_storage、desktop/plugins/
    /// MCP 等）合并进新备份；否则每次 hot-switch 都用供应商存储配置整体替换备份，
    /// 释放接管时这些设置会被重置为默认。独立订阅的 `model` 重新固定为 provider
    /// 自有模型，防止聚合残留的 gpt 插槽污染备份/活配置。
    fn preserve_codex_live_user_settings(
        effective_settings: &mut Value,
        existing_live: &Value,
        provider: &Provider,
    ) -> Result<(), String> {
    ...
    fn apply_codex_proxy_toml_config_for_provider(
        ...
        // 聚合 Provider 没有自己的 config.toml：以现有 config.toml 为基底合成
        // custom provider 路由到本地代理。缺少 model_provider 会让 Codex 直接
        // 请求 api.openai.com（残留 auth.json key → 401），代理根本收不到请求；
        // 而丢弃现有配置会把用户设置（语言、插件、MCP 等）重置为默认。
        if let Some(aggregate) = provider.filter(|p| p.is_aggregate()) {
            return crate::codex_config::build_aggregate_takeover_toml(
                aggregate, proxy_url, toml_str,
            )
            .map_err(|e| format!("生成聚合 Provider Codex 接管配置失败: {e}"));
        }
    ...
    fn write_codex_takeover_live_for_provider(
        ...
        // 聚合 Provider 先按成员供应商目录补齐缺失的模型元数据（上下文窗口等），
        // 这样存量聚合即使没有重新保存，切换后 Codex 目录/缓存也带正确能力声明。
        let enriched = if provider.is_some_and(Provider::is_aggregate) {
            self.enrich_aggregate_config_with_member_meta(config)
        } else {
            config.clone()
        };

        self.write_codex_takeover_live_for_provider_inner(&enriched, provider)?;

        // 非官方 Provider 接管后同步重建 Codex 桌面端模型缓存（models_cache.json），
        // 保证桌面端模型列表显示第三方模型；切回官方或离开带模型目录的 Provider
        // 时恢复/清除缓存（幂等）。不需要 OpenAI 官方登录。
        crate::codex_config::sync_codex_models_cache_for_provider(
            provider.as_ref().and_then(|p| p.category.as_deref()),
            &enriched,
        )
        .map_err(|e| format!("写入 Codex models_cache 失败: {e}"))?;

        Ok(())
    }

    /// 聚合 Provider：把 `aggregateModels` 中每条映射对应的成员 `modelCatalog`
    /// 元数据（contextWindow / supportsParallelToolCalls / inputModalities /
    /// baseInstructions）合并进 `modelCatalog.models`，仅补齐缺失字段。
    fn enrich_aggregate_config_with_member_meta(&self, config: &Value) -> Value {
        let Some(rows) = config.get("aggregateModels").and_then(Value::as_array) else {
            return config.clone();
        };
        if config
            .get("modelCatalog")
        ...
        // 槽位（对外模型名）-> 成员目录元数据（取第一个能匹配到的成员）
        let mut meta_by_slot: std::collections::HashMap<String, Value> =
            std::collections::HashMap::new();
        let mut member_catalogs: std::collections::HashMap<
            String,
        ...
        for row in rows {
            ...
            let provider_id = row
                .get("providerId")
                .or_else(|| row.get("provider_id"))
                .and_then(Value::as_str)
                .unwrap_or_default();
            let upstream = row
                .get("upstreamModel")
                .or_else(|| row.get("upstream_model"))
                .and_then(Value::as_str)
                .unwrap_or(slot);
            if provider_id.is_empty() {
                continue;
            }
            let catalog_by_model = member_catalogs
                .entry(provider_id.to_string())
                .or_insert_with(|| {
                    let Ok(Some(member)) = self.db.get_provider_by_id(provider_id, "codex") else {
                        log::warn!(
                            "读取聚合成员 {provider_id} 的模型目录失败，聚合目录元数据可能缺失"
                        );
                    ...
                    member
                        ...
                        .map(|models| {
                            models
                                .iter()
                                .filter_map(|entry| {
                                    let id = entry.get("model").and_then(Value::as_str)?;
                                    Some((id.to_string(), entry.clone()))
                                })
                                .collect()
                        })
                        .unwrap_or_default()
                });
            if let Some(meta) = catalog_by_model.get(upstream) {
                meta_by_slot
        ...
        if meta_by_slot.is_empty() {
            return config.clone();
        }

        let mut enriched = config.clone();
        let Some(models) = enriched
        ...
        for entry in models {
            let Some(slot) = entry.get("model").and_then(Value::as_str) else {
                continue;
            };
    ...
    fn write_codex_takeover_live_for_provider_inner(
        &self,
        config: &Value,
        provider: Option<&Provider>,
    ) -> Result<(), String> {
    ...
    async fn start_with_takeover_ephemeral_port_writes_actual_live_url() {
        ...
        let provider = Provider::with_id(
            ...
            json!({
                ...
                "config": r#"model_provider = "p1"
model = "gpt-5.1"

[model_providers.p1]
name = "P1"
base_url = "https://api.p1.example/v1"
wire_api = "responses"
"#
            }),
            None,
...
"#,
        ...
        let live = std::fs::read_to_string(crate::codex_config::get_codex_config_path())
            .expect("read taken-over live config");
        assert!(
            live.contains(&format!("http://127.0.0.1:{}/v1", info.port)),
            "taken-over config must reference the actual resolved port"
        );
        assert!(
            !live.contains(":0"),
...
"#
    ...
    #[tokio::test]
    #[serial]
    async fn codex_takeover_aggregate_hot_switch_writes_custom_proxy_route_and_preserves_backup() {
        let _home = TempHome::new();
        crate::settings::reload_settings().expect("reload settings");
        ...
        // 原始 Live 配置（接管开始前用户自己的 config.toml）。
        crate::codex_config::write_codex_live_atomic(
...
"#,
        ...
        let mut member = Provider::with_id(
            "member-1".to_string(),
...
"#,
        ...
        member.category = Some("custom".to_string());
        ...
        let mut aggregate = Provider::with_id(
            ...
            json!({
                ...
                "aggregateModels": [
                    { "model": "gpt-5.4", "providerId": "member-1", "upstreamModel": "deepseek-v4-flash" }
                ]
            }),
            None,
        );
        ...
        db.set_current_provider("codex", "member-1")
            .expect("set current");
        crate::settings::set_current_provider(&AppType::Codex, Some("member-1"))
        ...
        service
            .hot_switch_provider("codex", "agg-1")
            .await
            .expect("hot switch to aggregate");

        let live = std::fs::read_to_string(crate::codex_config::get_codex_config_path())
            .expect("read aggregate takeover config");
        let parsed: toml::Value = toml::from_str(&live).expect("valid TOML");
        assert_eq!(
            parsed["model_provider"].as_str(),
            Some("custom"),
            "aggregate takeover config must declare model_provider=custom so Codex routes through the proxy (regression: api.openai.com 401)"
        );
        let custom = &parsed["model_providers"]["custom"];
        assert_eq!(
            custom["base_url"].as_str(),
            Some(running_codex_base_url(&service).await.as_str()),
            "custom provider must point at the local proxy"
        );
        ...
        // 热切换到聚合 Provider 不应覆盖接管备份（原始 Live 配置）。
        let backup = db
        ...
        let backup_toml = backup_config["config"].as_str().expect("backup config");
        assert!(
            backup_toml.contains("previous.example"),
            "aggregate hot switch must keep the original backup, got: {backup_toml}"
        );

        // 聚合热切换提交后必须立即替换运行态目标。下一次请求不能继续使用
        // 切换前的单 Provider，更不能继承它的模型或凭据。
        let aggregate_status = service.get_status().await.expect("get aggregate status");
        assert!(
            aggregate_status
                .active_targets
                .iter()
                .any(|target| { target.app_type == "codex" && target.provider_id == "agg-1" }),
            "aggregate hot switch must update the runtime target immediately"
        );

        service
            .hot_switch_provider("codex", "member-1")
            .await
            .expect("hot switch back to member");
        let member_status = service.get_status().await.expect("get member status");
        assert!(member_status
            .active_targets
            .iter()
            .any(|target| { target.app_type == "codex" && target.provider_id == "member-1" }));
        let member_live = std::fs::read_to_string(crate::codex_config::get_codex_config_path())
            .expect("read member takeover config");
        let member_toml: toml::Value = toml::from_str(&member_live).expect("valid member TOML");
        assert_eq!(
            member_toml.get("model").and_then(|value| value.as_str()),
            Some("deepseek-v4-flash")
        );
        assert_eq!(
            member_toml["model_providers"]["custom"]["base_url"].as_str(),
            Some(running_codex_base_url(&service).await.as_str())
        );
        let catalog_path = member_toml
            .get("model_catalog_json")
            .and_then(|value| value.as_str())
            .expect("member catalog pointer");
        let member_catalog: Value = serde_json::from_str(
            &std::fs::read_to_string(catalog_path).expect("read member catalog"),
        )
        .expect("parse member catalog");
        let member_models: Vec<&str> = member_catalog["models"]
            .as_array()
            .expect("member models")
            .iter()
        ...
        assert_eq!(
            ...
            "member catalog must not retain aggregate GPT slots"
        );
        let member_backup = db
            .get_live_backup("codex")
            .await
            .expect("read member backup")
            .expect("member backup exists");
        let member_backup: Value =
            serde_json::from_str(&member_backup.original_config).expect("parse member backup");
        assert!(
            member_backup["config"]
                .as_str()
                .is_some_and(|config| config.contains("opencode.ai")),
            "switching to a regular provider must replace the pre-aggregate restore backup"
        );

        // 关闭接管应恢复当前普通 Provider 的独立配置，而不是聚合空配置。
        service
            .set_takeover_for_app("codex", false)
            .await
            .expect("disable takeover");
        ...
        assert!(
            restored.contains("opencode.ai"),
            "restore must bring back the original config, got: {restored}"
        );
    }
    ...
    #[tokio::test]
    #[serial]
    async fn codex_sync_live_does_not_overwrite_existing_provider_key() {
        let _home = TempHome::new();
        crate::settings::reload_settings().expect("reload settings");
        let db = Arc::new(Database::memory().expect("init db"));
        let service = ProxyService::new(db.clone());
        let provider = Provider::with_id(
            ...
            "OpenCode".to_string(),
            json!({
                "auth": { "OPENAI_API_KEY": "opencode-key" },
                "config": r#"model_provider = "custom"
model = "deepseek-v4-flash"
...
"#
        ...
        service
            .sync_live_config_to_provider(
                &AppType::Codex,
                &json!({
                    "auth": { "OPENAI_API_KEY": "unrelated-live-key" },
        ...
        let stored = db
            .get_provider_by_id("opencode", "codex")
        ...
        assert_eq!(
            stored
                ...
                .and_then(Value::as_str),
            Some("opencode-key"),
            "a stale Live key must not replace the provider-owned credential"
        );
    }

    #[tokio::test]
    #[serial]
    async fn codex_sync_live_backfills_provider_only_when_key_is_missing() {
        let _home = TempHome::new();
        crate::settings::reload_settings().expect("reload settings");
        let db = Arc::new(Database::memory().expect("init db"));
        let service = ProxyService::new(db.clone());
...
"#
        ...
        db.save_provider("codex", &provider).expect("save provider");
        db.set_current_provider("codex", "legacy")
            .expect("set current");
        crate::settings::set_current_provider(&AppType::Codex, Some("legacy"))
            .expect("set local current");
        ...
        service
            .sync_live_config_to_provider(
                &AppType::Codex,
                &json!({
                    "auth": { "OPENAI_API_KEY": "imported-live-key" },
                    "config": r#"model_provider = "legacy"
...
"#
        ...
        assert_eq!(
            stored
                ...
                .pointer("/auth/OPENAI_API_KEY")
                .and_then(Value::as_str),
            Some("imported-live-key"),
            "legacy rows without a key should still be recoverable from Live"
        );
    }

    #[tokio::test]
    #[serial]
    async fn codex_sync_live_does_not_fill_missing_key_from_different_upstream() {
        let _home = TempHome::new();
        crate::settings::reload_settings().expect("reload settings");
        let db = Arc::new(Database::memory().expect("init db"));
        let service = ProxyService::new(db.clone());
...
"#
        ...
        service
            .sync_live_config_to_provider(
                &AppType::Codex,
                &json!({
                    "auth": { "OPENAI_API_KEY": "other-provider-key" },
...
"#
        ...
        assert!(
            crate::codex_config::extract_codex_api_key(
                ...
                stored.settings_config.get("config").and_then(Value::as_str),
            )
            .is_none(),
            "a key from another upstream must not be imported"
        );
    }
...
"#;
    ...
    #[test]
    fn apply_codex_proxy_toml_config_synthesizes_aggregate_proxy_route() {
        let mut provider = Provider::with_id(
            "agg-1".to_string(),
            "My Aggregate".to_string(),
            json!({
        ...
        assert_eq!(
            parsed["model_provider"].as_str(),
            Some("custom"),
            "aggregate live config must declare a custom model provider, otherwise Codex falls back to api.openai.com"
        );
        assert_eq!(
            parsed["model"].as_str(),
            Some("gpt-5.5@neko"),
        ...
        let custom = &parsed["model_providers"]["custom"];
        assert_eq!(custom["base_url"].as_str(), Some(proxy_url));
        assert_eq!(custom["wire_api"].as_str(), Some("responses"));
        assert!(
            parsed.get("base_url").is_none(),
    ...
    fn apply_codex_proxy_toml_config_routes_builtin_official_with_native_auth() {
        ...
        let route_id = crate::codex_config::CODEX_OFFICIAL_PROXY_PROVIDER_ID;
...
"#;
    ...
    async fn switch_proxy_target_updates_live_backup_when_taken_over() {
        ...
        // 种子 Codex Live 配置（TempHome 提供全新 HOME，热切换路径需要读取 config.toml）
        crate::codex_config::write_codex_live_atomic(
            &json!({ "OPENAI_API_KEY": "a-key" }),
            Some("model_provider = \"a\"\nmodel = \"gpt-5.1\"\n"),
        )
        ...
        db.save_live_backup("codex", "{\"auth\":{}}")
            .await
        ...
        let backup = db
            ...
            .expect("backup exists");
        assert!(
            backup.original_config.contains("b-key"),
            "backup should carry the target provider's credentials"
        );
    }
    ...
    async fn hot_switch_provider_serializes_same_app_switches() {
        ...
        let backup = db
            ...
            .expect("backup exists");
        assert!(
            backup.original_config.contains("c-key"),
            "backup should carry the final provider's credentials"
        );
    }
    ...
    async fn restore_waits_for_hot_switch_and_restores_latest_backup() {
        ...
        .expect("seed live backup");
        crate::codex_config::write_codex_live_atomic(
            &provider_a.settings_config["auth"].clone(),
            provider_a
                .settings_config
                .get("config")
                .and_then(|v| v.as_str()),
        )
        .expect("seed live files");
        ...
        assert!(
            backup.original_config.contains("b-key"),
            "restore must observe the backup written by the hot switch"
        );
        ...
        assert_eq!(
            live.get("auth")
                ...
                .and_then(|v| v.as_str()),
            Some("b-key"),
            "restore must write the latest backup back to live"
        );
    }
...
"#
    ...
    #[tokio::test]
    #[serial]
    async fn update_live_backup_preserves_codex_live_user_settings_and_pins_provider_model() {
        let _home = TempHome::new();
        crate::settings::reload_settings().expect("reload settings");
        ...
        // Live config 处于接管投影状态：model 是聚合残留的 gpt 插槽，同时带
        // 用户在被接管期间保存的偏好设置。
...
[projects."/Users/test/codex-cube"]
trust_level = "trusted"
"#;
        crate::codex_config::write_codex_live_config_atomic(Some(live_config))
...
"#,
        ...
        assert!(
            config.contains("model_reasoning_effort = \"max\"")
                && config.contains("personality = \"pragmatic\""),
            "live user settings must survive hot-switch backup update; got: {config}"
        );
        assert!(
            config.contains("default_permissions = \":danger-full-access\"")
                && config.contains("approval_policy = \"never\"")
                && config.contains("[projects.\"/Users/test/codex-cube\"]")
                && config.contains("trust_level = \"trusted\""),
            "Codex access settings must survive hot-switch backup update; got: {config}"
        );
        assert!(
            config.contains("model = \"deepseek-v4-flash\""),
            "provider's own model must be pinned instead of stale aggregate slot; got: {config}"
        );
        assert!(
            !config.contains("model = \"gpt-5.4\""),
            "stale aggregate model must not leak into backup; got: {config}"
        );
    }
...
"#
    ...
    #[tokio::test]
    #[serial]
    async fn hot_switch_codex_provider_rewrites_stale_custom_thread_models() {
        let _home = TempHome::new();
        crate::settings::reload_settings().expect("reload settings");

        // Seed a Codex state DB with a stale `custom` thread left over from a
...
"#
        ...
        assert_eq!(
            ...
            "stale custom thread model must follow the hot-switched provider"
        );
        assert_eq!(
            rows[1],
...
"#;
        ...
        assert!(
            catalog_path.exists(),
            "codex-cube-model-catalog.json must be created on provider switch"
        );
...
"#;
    ...
    async fn codex_restore_from_backup_preserves_model_catalog_pointer() {
        ...
        // Pre-takeover Live state: config.toml points at the codex-cube generated
        ...
        assert!(
            ...
            "restored pointer must still reference the codex-cube generated catalog file"
        );
    }
    ...
    async fn codex_restore_from_backup_projects_inline_model_catalog() {
        ...
        assert!(
            catalog_path.exists(),
            "restore must generate the codex-cube catalog file on disk"
        );
    ...
    async fn codex_restore_empty_auth_backup_still_projects_inline_catalog() {
        ...
        assert!(
            ...
            "empty-auth restore must generate the codex-cube catalog file"
        );
        assert!(
    ...
    async fn restore_falls_through_to_ssot_when_backup_is_proxy_placeholder() {
        ...
        let provider = Provider::with_id(
            ...
            json!({
                "auth": {
                    "OPENAI_API_KEY": "real-key-from-db"
                },
                "config": r#"model_provider = "p1"
...
"#
        ...
        db.save_live_backup("codex", &corrupted_backup)
            .await
...
"#,
        ...
        .expect("seed taken-over live files");
        ...
        let restored_live = service.read_codex_live().expect("read live");
        assert_eq!(
            restored_live
                .get("auth")
                .and_then(|auth| auth.get("OPENAI_API_KEY"))
        ...
        let restored_config = restored_live
            .get("config")
            .and_then(|v| v.as_str())
            .expect("live config text");
        assert!(
            restored_config.contains("https://api.minimaxi.com/v1"),
    ...
    async fn backup_skips_when_live_is_already_proxy_placeholder() {
        ...
        let good_backup = serde_json::to_string(&json!({
            "auth": {
                "OPENAI_API_KEY": "real-codex-token"
            },
            "config": r#"model_provider = "custom"
...
"#
        ...
        db.save_live_backup("codex", &good_backup)
            .await
...
"#,
    ...
    async fn bulk_backup_skips_all_when_live_is_proxy_placeholder() {
        ...
        // Seed a good Codex backup (the "real" original Live)
        let good_backup = serde_json::to_string(&json!({
        ...
        // Seed the Codex Live files with proxy placeholders
        let codex_dir = crate::codex_config::get_codex_config_dir();
        std::fs::create_dir_all(&codex_dir).expect("create codex dir");
        std::fs::write(
...
"#,
        ...
        // Call bulk backup: must skip
        service
            .backup_live_configs()
            .await
            .expect("bulk backup should succeed (no-op when live is a placeholder)");
    ...
    #[test]
    #[serial]
    fn enrich_aggregate_config_with_member_meta_fills_missing_context_window() {
        let _home = TempHome::new();
        crate::settings::reload_settings().expect("reload settings");
        ...
        // 成员供应商目录：deepseek-v4-flash 声明 1M 上下文
            ...
            json!({
                "modelCatalog": {
                    "models": [
                        ...
                        { "model": "glm-5", "displayName": "GLM-5" }
                    ]
                }
            }),
            None,
        );
        db.save_provider("codex", &member)
            .expect("save member provider");

        // 存量聚合快照：缺 contextWindow / supportsParallelToolCalls
        let config = json!({
            "memberProviderIds": ["default"],
            "aggregateModels": [
                {
                    "model": "deepseek-v4-flash",
                    "providerId": "default",
        ...
        let enriched = service.enrich_aggregate_config_with_member_meta(&config);
        let models = enriched["modelCatalog"]["models"]
            .as_array()
            .expect("models array");
        let flash = models
            .iter()
        ...
        assert_eq!(flash["supportsParallelToolCalls"], true);
        // 已存在的显示名不被覆盖
        assert_eq!(flash["displayName"], "deepseek-v4-flash");
