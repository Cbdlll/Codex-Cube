...
impl StreamCheckService {
    ...
    /// 没有 Codex Cube 能可靠探测的目标——这类供应商的连通检测按钮在前端已隐藏
...
mod tests {
    ...
    fn test_resolve_base_url_uses_explicit_url_or_errors_when_missing() {
        // 有显式 base_url → 直接用（Codex adapter 提取）
        let p = make_provider(serde_json::json!({ "base_url": "https://relay.example/v1" }));
        assert_eq!(
        ...
        assert!(StreamCheckService::resolve_base_url(&AppType::Codex, &empty).is_err());
