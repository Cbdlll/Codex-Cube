...
use crate::commands::{CodexOAuthState, XaiOAuthState};
...
fn apply_forward_model_mapping(
    app_type: &AppType,
    provider: &Provider,
    body: &Value,
...
pub struct ForwardError {
    ...
    /// 实际发往上游的模型名（所有映射/改写之后）。仅在请求已构造完成并发出、
    /// 上游以错误状态码响应时才有值；此前失败的请求（如认证失败）为 None。
    pub outbound_model: Option<String>,
}

/// 单次转发失败：在 `ProxyError` 之外携带本次尝试实际发往上游的模型名，
/// 供错误响应精确展示「真正请求的模型」，避免把 Provider 配置模型误当实际请求。
pub struct ForwardAttemptError {
    pub error: ProxyError,
    pub outbound_model: Option<String>,
}

impl From<ProxyError> for ForwardAttemptError {
...
impl RequestForwarder {
    ...
    async fn forward_with_retry_inner(
        ...
        let mut last_outbound_model = None;
        let mut attempted_providers = 0usize;

        // 单 Provider 场景下跳过熔断器检查（故障转移关闭时）
        let is_single_provider = providers.len() == 1;
        let bypass_circuit_breaker = is_single_provider;

        // 单 Provider（故障转移关闭）：按 max_attempts 展开同一 Provider，让瞬态
        // 传输错误（连接失败/超时）能自动重试；其余可重试错误（5xx/429 等）在
        // Retryable 分支对单 Provider 直接终止，避免对同一上游反复轰炸。多
        // Provider 的故障转移语义完全不变。日志仍按原始 Provider 数量归类：
        // 单 Provider 重试不改变 FWD-003（SINGLE_PROVIDER_FAILED）语义。
        let original_provider_count = providers.len();
        let providers = if is_single_provider && self.max_attempts > 1 {
            (0..self.max_attempts)
                .map(|_| providers[0].clone())
        ...
        for provider in providers.iter() {
                ...
                .await
            {
                ...
                Err(e) => {
                    if self.media_retry_should_trigger(
                        ...
                        if replaced_images > 0 {
                                ...
                                .await
                            {
                                ...
                                Err(retry_err) => {
                                    let media_outbound_model = retry_err.outbound_model.clone();
                                    log::warn!(
                                    ...
                                    if let Some(mut err) = self
                                        ...
                                        .await
                                    {
                                        err.outbound_model = media_outbound_model;
                                        return Err(err);
                                    }
                                    last_outbound_model = media_outbound_model;
                                    // 单 Provider：媒体降级重试失败后不再对同一
                                    // Provider 重复尝试（原行为是立即失败）。
                    ...
                    let category = self.categorize_proxy_error(&e.error, provider);
                    ...
                    match category {
                        ErrorCategory::Retryable => {
...

                            {
                                let mut status = self.status.write().await;
                                status.last_error =
                                    Some(format!("Provider {} 失败: {}", provider.name, e.error));
                            }
                            ...
                            // 单 Provider（故障转移关闭）：只对瞬态传输错误（连接
                            // 失败/超时）自动重试同一 Provider；5xx/429/配额等其余
                            // 可重试错误对同一上游重试无益，保持立即失败。重试前
                            // 小幅退避，让抖动中的网络/上游有时间恢复。
                            let single_provider_abort = is_single_provider
                                && !matches!(
                                    e.error,
                                    ProxyError::Timeout(_) | ProxyError::ForwardFailed(_)
                                );
                            last_error = Some(e.error);
                            last_outbound_model = e.outbound_model;
                            last_provider = Some(provider.clone());

                            if single_provider_abort {
                                break;
                            }
                            if is_single_provider {
                                tokio::time::sleep(std::time::Duration::from_millis(500)).await;
                            }
                            // 继续尝试下一个供应商（单 Provider 时为同一 Provider 的下一轮）
                            continue;
                        }
                        ErrorCategory::NonRetryable | ErrorCategory::ClientAbort => {
                                ...
                                .await;
                            {
                                let mut status = self.status.write().await;
                                status.failed_requests += 1;
                                status.last_error = Some(e.error.to_string());
                                if status.total_requests > 0 {
                                    status.success_rate = (status.success_requests as f32
                                        / status.total_requests as f32)
                                        * 100.0;
                            ...
                            return Err(ForwardError {
                                error: e.error,
                                provider: Some(provider.clone()),
                                outbound_model: e.outbound_model,
                            });
                        }
                    }
                }
    ...
    async fn forward(
    ...
    ) -> Result<(ProxyResponse, Option<String>, Option<String>), ForwardAttemptError> {
        // 使用适配器提取 base_url
        ...
        // 普通 Codex native Responses 请求以客户端 body 的 model 为准，不应用
        // Provider 默认/环境模型映射（目录支持外的过期 model 会在下方按
        // apply_codex_upstream_model 回退）。聚合 forced route 会在下方按显式
        // model → member → upstreamModel 映射覆盖；Chat/Anthropic 转换路径仍保留
        // 原有模型映射与协议转换行为。
        let mapped_body = apply_forward_model_mapping(
            app_type,
            provider,
            body,
            codex_responses_to_chat,
        ...
        if !codex_responses_to_anthropic {
            ...
            // model-catalog match (apply_codex_anthropic_upstream_model) and the transform's own
        ...
        // Codex 不经过 Claude/Gemini 格式分发：上游格式由 adapter 决定。
        let resolved_claude_api_format: Option<String> = None;
        ...
        let mut request_body = if codex_responses_to_chat {
            ...
            let mut mapped_body = mapped_body;
            super::providers::apply_codex_anthropic_upstream_model(provider, &mut mapped_body);
            ...
            // added. apply_codex_anthropic_upstream_model may have written back a model
        ...
        if matches!(app_type, AppType::Codex)
            ...
            && super::providers::transform_codex_responses_namespace::prepare_collaboration_message_schemas(
                &mut request_body,
            )
        {
            log::debug!(
                "[Codex] Removed encrypted collaboration message schema markers for native Responses upstream (provider={})",
                provider.id
            );
        }
        ...
        // xAI native-Responses path: scrub the OpenAI-backend-private fields
        ...
        // 第三方中转站（非官方 OpenAI，如各中转 API）会严格校验 Responses 历史的
        // 工具调用配对：function_call / custom_tool_call 与对应的 *_output 必须在
        // 同一请求内成对，web_search_call 不接受 output 字段。Codex 按上下文窗口
        // 裁剪历史时会留下孤立的 output（或孤立 call），中转站直接 400
        // `Upstream request failed`。透传前剔除孤立项，官方 OpenAI 不受影响。
        // Provider 可通过 settings_config.relaySanitize=false 显式关闭此清洗。
        let relay_sanitize_opted_out = provider
            .settings_config
            .get("relaySanitize")
        ...
        if matches!(app_type, AppType::Codex)
            ...
            && !codex_responses_to_anthropic
            && !super::providers::is_codex_official_provider(provider)
            && !relay_sanitize_opted_out
        {
            let reasoning_stats = if log::log_enabled!(log::Level::Debug) {
                super::providers::transform_codex_responses_relay_sanitize::summarize_relay_reasoning_history(
                    &request_body,
                )
            } else {
                None
            ...
            let sanitized =
                super::providers::transform_codex_responses_relay_sanitize::sanitize_relay_responses_request(
                    &mut request_body,
                    super::providers::transform_codex_responses_relay_sanitize::relay_profile_for_url(
                        &url,
                    ),
                );
            if sanitized {
                log::warn!(
                    "[Codex] Sanitized relay-incompatible Responses history (provider={}, reasoning={})",
                    provider.id,
                    format_reasoning_history_stats(reasoning_stats.as_ref()),
                );
            } else if let Some(stats) = reasoning_stats.as_ref() {
                log::debug!(
                    "[Codex] Relay Responses reasoning history (provider={}, {})",
                    provider.id,
                    format_reasoning_history_stats(Some(stats)),
                );
            }
        }

        // 原生 Responses 直连：Provider 目录明确支持的 model 原样转发；目录外的
        // 过期 model（切换供应商前的旧会话）回退到 Provider 配置的上游模型。
        // 聚合 Provider 的显式 model → member → upstreamModel 路由按 forced route
        // marker 无条件改写。Chat/Anthropic 协议转换分支已各自处理所需上游模型。
        if matches!(app_type, AppType::Codex)
            && !codex_responses_to_chat
            && !codex_responses_to_anthropic
        {
        ...
        for (key, value) in headers {
                ...
                || key_str.eq_ignore_ascii_case("x-goog-api-key")
            {
                ...
                // credential in Codex-Cube. `requires_openai_auth = true` makes
        ...
        // 输出请求信息日志：同时记录映射前的入站模型与定稿后的出站模型。
        // 单一 `(model=...)` 在模型被改写（过期会话回退 / 聚合路由 /
        // 协议转换）时无法区分两者，容易把改写后的模型误诊为客户端所选。
        let tag = adapter.name();
        let request_model = body
            .get("model")
            .and_then(|v| v.as_str())
            .unwrap_or("<none>");
        let outbound_model_log = filtered_body
            .get("model")
            .and_then(|v| v.as_str())
            .unwrap_or("<none>");
        log::info!(
            "[{tag}] >>> 请求目标: {target_for_log} request_model={request_model}, outbound_model={outbound_model_log}"
        );
        log::debug!(
        ...
        let preserve_exact_header_case =
            should_preserve_exact_header_case(adapter.name(), provider);
...
fn should_preserve_exact_header_case(adapter_name: &str, provider: &Provider) -> bool {
    if matches!(adapter_name, "Codex") {
        return false;
    }
...
fn map_reqwest_send_error(error: reqwest::Error) -> ProxyError {
    // 附带（脱敏后的）上游 origin，让 502 的错误信息能明确区分「上游不可达」
    // 与「本地代理自环」：Codex 客户端看到的是它自己请求的本地地址
    // (127.0.0.1:15721)，不带上游信息时极易误判为代理自环。
    // 只显示 origin（scheme://host[:port]），避免 path/query 中的密钥泄漏。
    let upstream_hint = error
        .url()
        .map(|u| format!(" (上游: {})", crate::redact_url_origin_for_log(u.as_str())))
        .unwrap_or_default();
    if error.is_timeout() {
        ProxyError::Timeout(format!(
            "上游请求超时: {}{}",
...
fn format_reasoning_history_stats(
    stats: Option<
        &super::providers::transform_codex_responses_relay_sanitize::ReasoningHistoryStats,
    >,
) -> String {
    ...
    format!(
        "total={},valid_text={},missing_content={},encrypted={},summary_only={}",
        stats.reasoning_total,
        stats.reasoning_text_valid,
        stats.content_missing_null_empty,
        stats.encrypted_content_present,
        stats.summary_only,
    )
}
...
mod tests {
    ...
    #[test]
    fn native_codex_forward_mapping_preserves_request_model() {
        let mut provider = test_provider_with_type(None);
        provider.settings_config = json!({
            "env": { "ANTHROPIC_MODEL": "provider-default-model" }
        });
        let body = json!({ "model": "gpt-5.6-sol", "input": "hi" });

        let mapped = apply_forward_model_mapping(&AppType::Codex, &provider, &body, false, false);

        assert_eq!(mapped["model"].as_str(), Some("gpt-5.6-sol"));
    }

    #[test]
    fn codex_protocol_conversion_keeps_existing_model_mapping_behavior() {
        let mut provider = test_provider_with_type(None);
        provider.settings_config = json!({
            "env": { "ANTHROPIC_MODEL": "provider-default-model" }
        });
        ...
        let chat = apply_forward_model_mapping(&AppType::Codex, &provider, &body, true, false);
        let anthropic = apply_forward_model_mapping(&AppType::Codex, &provider, &body, false, true);

        assert_eq!(chat["model"].as_str(), Some("provider-default-model"));
        assert_eq!(anthropic["model"].as_str(), Some("provider-default-model"));
    }
    ...
    /// 通过真实 `RequestForwarder::forward()` 发送请求到本地 TCP 服务器，
    /// 捕获实际序列化后的 HTTP JSON body，验证模型映射贯穿最终序列化。
    /// Provider 是配置了 `deepseek-v4-flash` 上游模型、目录为 `catalog` 的
    /// 普通第三方 Responses Provider，base_url 指向本测试服务器。
    /// 返回 `(捕获的请求 body, claude_api_format, outbound_model)`。
    async fn run_forward_capture_test(
        catalog: &[&str],
        incoming_model: &str,
        ...
        let provider = native_responses_provider(&format!("http://{addr}"), catalog);
        ...
        let server = tokio::spawn(async move {
            ...
            let content_length = loop {
                ...
                assert!(n > 0, "upstream closed before full request arrived");
                buf.extend_from_slice(&chunk[..n]);
                let text = String::from_utf8_lossy(&buf);
                if let Some(header_end) = text.find("\r\n\r\n") {
                    let header_text = &text[..header_end];
                    if let Some(len) = header_text.lines().find_map(|line| {
            ...
            let body: Value =
                serde_json::from_slice(&body_bytes).expect("parse upstream request body");
        ...
        let body = json!({ "model": incoming_model, "input": "hello", "stream": false });
            ...
            .await
        {
            Ok(result) => result,
            Err(error) => {
                // 转发失败时终止仍在等待 accept 的服务器任务，避免悬挂。
                server.abort();
                panic!("forward failed: {}", error.error);
            }
        };

        let captured = tokio::time::timeout(Duration::from_secs(5), server)
            .await
            .expect("local server task timed out")
            .expect("local server task panicked");
        (captured, result.1, result.2)
    }

    fn native_responses_provider(base_url: &str, catalog: &[&str]) -> Provider {
        let catalog_models: Vec<Value> = catalog
            .iter()
    ...
    #[tokio::test]
    async fn forward_native_responses_rewrites_stale_model_through_final_serialization() {
        let (captured, _claude_api_format, outbound_model) =
            run_forward_capture_test(&["deepseek-v4-flash"], "gpt-5.6-luna").await;

        assert_eq!(
            captured["model"].as_str(),
            Some("deepseek-v4-flash"),
            "serialized upstream body must carry the configured upstream model"
        );
        assert_eq!(
            outbound_model.as_deref(),
            Some("deepseek-v4-flash"),
            "returned outbound_model must match the final serialized body model"
        );
    }

    #[tokio::test]
    async fn forward_native_responses_preserves_catalog_supported_model_through_final_serialization(
    ) {
        let (captured, _claude_api_format, outbound_model) =
            run_forward_capture_test(&["deepseek-v4-flash", "deepseek-v4-pro"], "deepseek-v4-pro")
                .await;

        assert_eq!(
            captured["model"].as_str(),
            Some("deepseek-v4-pro"),
            "catalog-supported request model must survive final serialization"
        );
        assert_eq!(
            outbound_model.as_deref(),
            Some("deepseek-v4-pro"),
            "returned outbound_model must match the preserved catalog model"
        );
    }
    ...
    fn single_provider_retryable_log_uses_single_provider_code() {
        ...
        let (code, message) = build_retryable_failure_log("TestProvider-response", 1, 1, &error);

        assert_eq!(code, log_fwd::SINGLE_PROVIDER_FAILED);
        assert!(message.contains("Provider TestProvider-response 请求失败"));
    ...
    fn local_proxy_header_overrides_replace_allowed_headers_only() {
        ...
        apply_local_proxy_header_overrides(&mut headers, Some(&overrides));
    ...
    #[test]
    fn exact_header_case_preserved_for_non_codex() {
        let provider = test_provider_with_type(None);

        assert!(should_preserve_exact_header_case("Claude", &provider));
        assert!(!should_preserve_exact_header_case("Codex", &provider));
    }

    #[test]
    fn exact_header_case_skipped_for_codex_oauth_and_xai_oauth() {
        let codex_oauth = test_provider_with_type(Some("codex_oauth"));
        let xai_oauth = test_provider_with_type(Some("xai_oauth"));

        assert!(!should_preserve_exact_header_case("Claude", &codex_oauth));
        assert!(!should_preserve_exact_header_case("Claude", &xai_oauth));
    }
