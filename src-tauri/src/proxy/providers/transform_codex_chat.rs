...
//!
//! This module is used when the Codex client talks to Codex-Cube through the
...
impl CodexToolContext {
    ...
    fn add_chat_tool(&mut self, chat_name: String, spec: CodexToolSpec, chat_tool: Value) {
        ...
        // Chat Completions requires every function tool to carry a non-empty
        // nested `function.name`. A malformed Responses tool must not reach a
        // strict gateway as `function: {}`/`function: null`.
        let valid_function_name = chat_tool
            .pointer("/function/name")
            .and_then(Value::as_str)
            .is_some_and(|name| !name.trim().is_empty());
        if !valid_function_name {
            log::warn!("[Codex] dropping malformed tool `{chat_name}` without a function name");
            return;
        }
...
fn responses_function_tool_to_chat_tool(tool: &Value, chat_name: &str) -> Option<Value> {
    ...
    if let Some(function @ Value::Object(_)) = tool.get("function") {
...
fn responses_tool_choice_to_chat(tool_choice: &Value, tool_context: &CodexToolContext) -> Value {
    match tool_choice {
        Value::Object(obj) if obj.get("type").and_then(|v| v.as_str()) == Some("function") => {
            ...
            let chat_name = tool_context.chat_name_for_response_function(name, namespace);
            if !tool_context.seen_chat_names.contains(&chat_name) {
                log::warn!(
                    "[Codex] tool_choice 引用已被丢弃的工具 `{chat_name}`，移除 tool_choice"
                );
                return Value::Null;
            }
            json!({
        ...
        Value::Object(obj) if obj.get("type").and_then(|v| v.as_str()) == Some("custom") => {
            let name = obj.get("name").and_then(|v| v.as_str()).unwrap_or("");
            if !tool_context.seen_chat_names.contains(name) {
                log::warn!("[Codex] tool_choice 引用已被丢弃的工具 `{name}`，移除 tool_choice");
                return Value::Null;
            }
            json!({
                "type": "function",
...
mod tests {
    ...
    fn large_test_image_data_url() -> String {
        let bytes = b"CODEX_CUBE_TOOL_MEDIA_SENTINEL".repeat(400);
    ...
    #[test]
    fn responses_request_to_chat_recovers_name_when_function_is_null() {
        let input = json!({
        ...
        assert_eq!(result["tools"][0]["function"]["name"], "exec_command");
    }

    #[test]
    fn responses_request_to_chat_drops_function_tool_without_name() {
        let input = json!({
            "model": "deepseek-v4-pro",
            "tools": [{
                "type": "function",
                "function": {"description": "malformed"}
            }],
            "input": "hi"
        });
    ...
    fn responses_request_to_chat_moves_tool_image_to_synthetic_user_message() {
        ...
        assert_eq!(
            ...
            "[codex-cube: media output of tool call call_image]"
        );
    ...
    fn responses_request_to_chat_clamps_stringified_custom_output_residual_base64() {
        ...
        assert!(rewritten.contains("[codex-cube: omitted 20000 bytes]"));
    ...
    fn responses_request_to_chat_clamps_only_residual_base64ish_strings() {
        ...
        assert!(tool_content[2]["data"]
            .as_str()
            .unwrap()
            .starts_with("[codex-cube: omitted 20000 bytes]"));
    ...
    // https://github.com/Cbdlll/Codex-Cube/issues/3557
