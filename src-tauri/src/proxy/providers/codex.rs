...
/// Switch through the Responses API, so Codex-Cube bridges Responses ⇄ Anthropic.
///
...
fn normalize_model_api_format(value: &str) -> Option<&'static str> {
    match value.trim().to_ascii_lowercase().as_str() {
        "openai_responses" | "responses" => Some("openai_responses"),
        "openai_chat" | "chat" | "chat_completions" | "completions" => Some("openai_chat"),
        "anthropic" | "messages" => Some("anthropic"),
        _ => None,
    }
}

/// Normalize any accepted wire_api / api_format alias to the canonical value
/// written into generated Codex config: `responses` | `chat` | `anthropic`.
/// Unknown values return `None` so callers fall back to `responses`.
/// Internal aliases (`openai_responses` / `chat_completions` / …) must never be
/// written verbatim into Codex config; Codex only understands canonical values.
pub fn normalize_codex_wire_api(value: &str) -> Option<&'static str> {
    ...
    match value.to_ascii_lowercase().as_str() {
        "responses" | "openai_responses" | "openai-responses" => Some("responses"),
        _ => None,
    }
}
...
/// Whether this Codex provider speaks the native OpenAI Responses wire format
/// (i.e. neither Chat Completions nor Anthropic Messages conversion is needed).
...
/// 聚合 Provider 的按模型路由：请求模型命中 `aggregate.models` 条目时，返回
/// 绑定的成员供应商，并把成员配置里的 `model` 覆盖为条目指定的上游模型。
///
/// - 先精确匹配对外模型名（两侧剥离 `[1M]` 上下文后缀）。
/// - 未命中时做精确反向解析：请求模型等于某条目的 `upstreamModel`（如 worker
///   固定请求 `deepseek-v4-flash`）时命中自身条目，不能落入 first-entry fallback。
/// - 命中后不参与故障转移：调用方直接把该成员作为唯一候选，避免误触发切换。
/// - 每条映射可通过 `apiFormat` 覆盖成员 Provider 的上游协议。
pub fn resolve_codex_aggregate_provider(
    db: &Database,
    aggregate: &Provider,
    model: &str,
) -> Result<Option<Provider>, AppError> {
    let entries = crate::codex_config::codex_aggregate_model_entries(&aggregate.settings_config);
    ...
    let Some(mut provider) = db.get_provider_by_id(&entry.provider_id, "codex")? else {
        return Err(AppError::Message(format!(
            "Codex 聚合 Provider 的模型 `{model}` 绑定的供应商不存在（{}），请重新配置",
            entry.provider_id
        )));
    };

    // 成员没有声明上游模型时，用对外模型名的模型段兜底（去掉 @供应商 后缀），
    // 避免聚合模型名直接泄漏到上游导致 400。
    let member_lacks_upstream_model = codex_provider_upstream_model(&provider).is_none();
    if let Some(obj) = provider.settings_config.as_object_mut() {
        ...
        // apply_codex_upstream_model 无条件改写为上游模型。
        obj.insert("codex_cube_custom_route".to_string(), JsonValue::Bool(true));
    }

    if let Some(format) = entry
        .api_format
        .as_deref()
        .and_then(normalize_model_api_format)
    {
        provider
            .meta
    ...
    provider
        ...
        .or_else(|| {
            provider
                ...
                .and_then(|v| v.as_str())
                .and_then(|config| {
                    crate::codex_config::extract_model_config(config)
...
/// Whether aggregate routing explicitly injected an upstream-model mapping into
/// this in-memory provider clone. Ordinary providers never receive this marker.
fn codex_provider_has_forced_upstream_model_route(provider: &Provider) -> bool {
    provider
        .settings_config
...
/// Resolve the provider model for protocol-conversion paths (Codex Responses →
/// Chat Completions / Anthropic Messages). These upstream protocols may require
/// the provider's configured model name after conversion. A request model that
/// the provider catalog explicitly supports is preserved.
fn apply_codex_converted_upstream_model(
    provider: &Provider,
    body: &mut JsonValue,
) -> Option<String> {
    if !codex_provider_has_forced_upstream_model_route(provider) {
...
pub fn apply_codex_chat_upstream_model(
    ...
    apply_codex_converted_upstream_model(provider, body)
}

/// For Codex → Anthropic protocol conversion, resolve the model required by the
/// target provider while preserving an explicitly supported request model.
pub fn apply_codex_anthropic_upstream_model(
    provider: &Provider,
    body: &mut JsonValue,
) -> Option<String> {
    apply_codex_converted_upstream_model(provider, body)
...
/// Native Responses requests normally use the model selected by Codex verbatim.
///
/// An aggregate provider's explicit model → member → upstreamModel route, marked
/// on the in-memory member clone by `resolve_codex_aggregate_provider`, is
/// applied exactly (unconditional rewrite to the resolved upstream model).
///
/// For an ordinary provider, a request model that is NOT present in the
/// provider's model catalog is treated as a stale selection from a previously
/// active provider and falls back to the provider's configured upstream model.
/// This lets old conversations keep working after a provider switch instead of
/// 403-ing on a model the new provider does not serve. A model that the catalog
/// explicitly supports is preserved so Codex can still pick among them.
    ...
    fn extract_key(&self, provider: &Provider) -> Option<String> {
        ...
        if let Some(config) = provider.settings_config.get("config") {
            ...
            if let Some(config_str) = config.as_str() {
                if let Some((_, key)) = crate::codex_config::extract_credentials(config_str) {
                    return Some(key);
                }
                if let Some(key) =
...
impl ProviderAdapter for CodexAdapter {
    ...
    fn extract_base_url(&self, provider: &Provider) -> Result<String, ProxyError> {
        ...
        if let Some(config) = provider.settings_config.get("config") {
            ...
            // 尝试解析 TOML 字符串格式
            if let Some(config_str) = config.as_str() {
                if let Some(url) = crate::codex_config::extract_base_url(config_str) {
                    return Ok(url.trim_end_matches('/').to_string());
                }
...
"#
    ...
    #[test]
    fn test_normalize_codex_wire_api() {
        assert_eq!(normalize_codex_wire_api("responses"), Some("responses"));
        assert_eq!(
            normalize_codex_wire_api("openai_responses"),
            Some("responses")
        );
        assert_eq!(
            normalize_codex_wire_api("openai-responses"),
            Some("responses")
        );
        assert_eq!(normalize_codex_wire_api("chat"), Some("chat"));
        assert_eq!(normalize_codex_wire_api("chat_completions"), Some("chat"));
        assert_eq!(normalize_codex_wire_api("openai_chat"), Some("chat"));
        assert_eq!(normalize_codex_wire_api("anthropic"), Some("anthropic"));
        assert_eq!(
            normalize_codex_wire_api("anthropic_messages"),
            Some("anthropic")
        );
        assert_eq!(normalize_codex_wire_api("messages"), Some("anthropic"));
        assert_eq!(normalize_codex_wire_api("bogus"), None);
        assert_eq!(normalize_codex_wire_api(""), None);
    }
...
"#
    ...
    #[test]
    fn test_apply_codex_upstream_model_falls_back_for_unsupported_request_model() {
        // A request model that the current provider's catalog does not support is
        // treated as a stale selection from a previously-active provider and is
        // rewritten to the configured upstream model so the request does not 403.
        let provider = create_provider(json!({
            "config": r#"model_provider = "custom"
...
"#,
        ...
        let mut body = json!({ "model": "gpt-5.4", "input": "hi" });

        let result = apply_codex_upstream_model(&provider, &mut body);

        assert_eq!(result.as_deref(), Some("deepseek-v4-flash"));
        assert_eq!(
    ...
    #[test]
    fn test_apply_codex_upstream_model_preserves_catalog_model() {
        // A model the provider's catalog explicitly supports is preserved so Codex
        // can still select among the provider's served models.
        let provider = create_provider(json!({
            "config": r#"model_provider = "custom"
model = "deepseek-v4-flash"
...
"#,
        ...
        let mut body = json!({ "model": "deepseek-v4-pro", "input": "hi" });
        ...
        assert_eq!(
            ...
            "a supported native Responses model must be preserved"
        );
    }

    #[test]
    fn test_apply_codex_upstream_model_forced_aggregate_route_rewrites_model() {
        // Aggregate routing explicitly injects both the forced-route marker and
        // the resolved upstreamModel into the in-memory member clone.
        let provider = create_provider(json!({
            "codex_cube_custom_route": true,
            "model": "deepseek-v4-flash",
            "modelCatalog": {
                "models": [
                    { "model": "gpt-5.6-sol@worker" },
                    { "model": "deepseek-v4-flash" }
                ]
            }
        }));
        let mut body = json!({ "model": "gpt-5.6-sol@worker", "input": "hi" });
        ...
        assert_eq!(
            body.get("model").and_then(JsonValue::as_str),
            Some("deepseek-v4-flash"),
            "forced aggregate route must apply its explicit upstreamModel"
        );
    }

    #[test]
    fn test_apply_codex_anthropic_upstream_model_preserves_catalog_selection() {
        // Protocol conversion retains an explicitly supported model, including
        // the [1m] marker required by the Anthropic transform.
        let provider = create_provider(json!({
            "config": r#"model_provider = "custom"
...
"#,
        ...
        let result = apply_codex_anthropic_upstream_model(&provider, &mut body);
    ...
    #[test]
    fn test_apply_codex_anthropic_upstream_model_uses_provider_model_for_conversion() {
        // Anthropic is a protocol-conversion path, so an unsupported client model
        // is replaced by the provider's configured upstream model as before.
        let provider = create_provider(json!({
            "config": r#"model_provider = "custom"
...
"#
    ...
    #[test]
    fn namespace_flatten_gate_covers_strict_third_party_responses() {
        ...
        // Plain third-party native Responses gateways also reject Codex's
        // private namespace tool shape and must be flattened.
        ...
        let mut managed = create_provider(json!({
            "auth": {},
        ...
        assert!(!provider_needs_responses_namespace_flatten(&managed));
    }

    #[test]
    fn aggregate_resolves_member_and_forces_upstream_model() {
        let db = Database::memory().expect("memory db");
        let mut member = create_provider(json!({
            "auth": { "OPENAI_API_KEY": "sk-deepseek" },
            "config": "base_url = \"https://api.deepseek.com/v1\"\nwire_api = \"responses\"",
            "model": "deepseek-chat",
            "modelCatalog": { "models": [{ "model": "deepseek-chat" }] }
        }));
        member.id = "deepseek".to_string();
        db.save_provider("codex", &member).expect("save member");

        let mut aggregate = create_provider(json!({
            "aggregateModels": [
                {
        ...
        let resolved = resolve_codex_aggregate_provider(&db, &aggregate, "deepseek-chat@deepseek")
            .expect("resolve aggregate route")
        ...
        assert_eq!(
            ...
            Some(true),
            "custom route marker must force the upstream rewrite"
        );

        // 请求体里的聚合插槽名必须被改写为上游模型（即使插槽名出现在成员目录里）
        let mut body = json!({ "model": "deepseek-chat@deepseek", "input": "hi" });
        let applied = apply_codex_upstream_model(&resolved, &mut body);
        assert_eq!(applied.as_deref(), Some("deepseek-chat"));
        assert_eq!(
            body.get("model").and_then(JsonValue::as_str),
            Some("deepseek-chat"),
            "aggregate model must be rewritten to the upstream model"
        );
    }

    #[test]
    fn aggregate_resolves_member_with_default_upstream_from_provider() {
        let db = Database::memory().expect("memory db");
        let mut member = create_provider(json!({
        ...
        member.id = "kimi".to_string();
        db.save_provider("codex", &member).expect("save member");

        let aggregate = create_provider(json!({
            "aggregateModels": [
                { "model": "kimi-k2", "providerId": "kimi" }
            ]
        }));
        let resolved = resolve_codex_aggregate_provider(&db, &aggregate, "kimi-k2[1M]")
            .expect("resolve with [1M] suffix")
            .expect("mapped member");
        ...
        assert_eq!(
            resolved
                ...
                .and_then(|v| v.as_str()),
            Some("kimi-k2"),
            "missing upstreamModel must fall back to the member's default model"
        );
    }

    #[test]
    fn aggregate_resolves_by_upstream_model_without_first_entry_fallback() {
        let db = Database::memory().expect("memory db");
        let mut coordinator_member = create_provider(json!({
            "auth": { "OPENAI_API_KEY": "sk-coordinator" },
            "config": "base_url = \"https://api.coordinator.example/v1\"\nwire_api = \"responses\"",
            "model": "gpt-5.6-sol"
        }));
        coordinator_member.id = "coordinator-member".to_string();
        db.save_provider("codex", &coordinator_member)
            .expect("save coordinator member");

        let mut worker_member = create_provider(json!({
            "auth": { "OPENAI_API_KEY": "sk-deepseek" },
            "config": "base_url = \"https://api.deepseek.com/v1\"\nwire_api = \"responses\"",
            "model": "deepseek-v4-flash"
        }));
        worker_member.id = "deepseek".to_string();
        db.save_provider("codex", &worker_member)
            .expect("save worker member");

        let mut aggregate = create_provider(json!({
            "aggregateModels": [
                {
                    "model": "gpt-5.6-sol",
                    "providerId": "coordinator-member",
        ...
        // 请求上游模型名（worker agent 固定的 deepseek-v4-flash）必须命中
        // deepseek 条目自身，而不是回退到列表第一条 coordinator 条目。
        let resolved = resolve_codex_aggregate_provider(&db, &aggregate, "deepseek-v4-flash")
            .expect("resolve aggregate route")
            .expect("mapped worker member");
        assert_eq!(resolved.id, "deepseek");
        assert_eq!(
            resolved
                .settings_config
        ...
        let mut body = json!({ "model": "deepseek-v4-flash", "input": "hi" });
        let applied = apply_codex_upstream_model(&resolved, &mut body);
        assert_eq!(applied.as_deref(), Some("deepseek-v4-flash"));
        assert_eq!(
            body.get("model").and_then(JsonValue::as_str),
            Some("deepseek-v4-flash")
        );
        ...
        // 对外插槽名仍按原逻辑精确命中。
        let coordinator = resolve_codex_aggregate_provider(&db, &aggregate, "gpt-5.6-sol")
            .expect("resolve aggregate route")
            .expect("mapped coordinator member");
        assert_eq!(coordinator.id, "coordinator-member");
    }

    #[test]
    fn aggregate_unmapped_model_returns_none() {
        let db = Database::memory().expect("memory db");
        let aggregate = create_provider(json!({
            "aggregateModels": [
        ...
        assert!(resolve_codex_aggregate_provider(&db, &aggregate, "gpt-5.5")
            .expect("resolve unmapped")
            .is_none());
    }

    #[test]
    fn aggregate_missing_member_provider_errors() {
        let db = Database::memory().expect("memory db");
        let aggregate = create_provider(json!({
            "aggregateModels": [
                { "model": "deepseek-chat", "providerId": "deleted-member", "upstreamModel": "deepseek-chat" }
            ]
        }));
        let result = resolve_codex_aggregate_provider(&db, &aggregate, "deepseek-chat");
        assert!(
            result.is_err(),
            "deleted member provider must surface a config error"
        );
    }

    #[test]
    fn aggregate_accepts_chat_member_and_inherits_protocol() {
        let db = Database::memory().expect("memory db");
        let mut chat_member = create_provider(json!({
            "auth": { "OPENAI_API_KEY": "sk-deepseek" },
            "config": "base_url = \"https://api.deepseek.com/v1\"\nwire_api = \"chat\"",
            "model": "deepseek-chat"
        }));
        chat_member.meta = Some(crate::provider::ProviderMeta {
            api_format: Some("openai_chat".to_string()),
            ..Default::default()
        });
        chat_member.id = "chat-member".to_string();
        db.save_provider("codex", &chat_member)
            .expect("save chat member");

        let aggregate = create_provider(json!({
            "aggregateModels": [
                { "model": "deepseek-chat", "providerId": "chat-member", "upstreamModel": "deepseek-chat" }
            ]
        }));
        let resolved = resolve_codex_aggregate_provider(&db, &aggregate, "deepseek-chat")
            .expect("resolve chat member")
            .expect("mapped chat member");
        assert!(codex_provider_uses_chat_completions(&resolved));
        assert_eq!(
            resolved
                .meta
                .as_ref()
    ...
    #[test]
    fn aggregate_model_api_format_explicitly_overrides_member_protocol() {
        let db = Database::memory().expect("memory db");
        let mut responses_member = create_provider(json!({
            "auth": { "OPENAI_API_KEY": "sk-upstream" },
            "config": "base_url = \"https://api.example.com/v1\"\nwire_api = \"responses\"",
            "model": "upstream-model"
        }));
        responses_member.meta = Some(crate::provider::ProviderMeta {
            api_format: Some("openai_responses".to_string()),
            ..Default::default()
        });
        responses_member.id = "responses-member".to_string();
        db.save_provider("codex", &responses_member)
            .expect("save responses member");

        let aggregate = create_provider(json!({
            "aggregateModels": [
                {
                    "model": "gpt-5.4",
                    "providerId": "responses-member",
                    "upstreamModel": "upstream-model",
                    "apiFormat": "openai_chat"
                }
            ]
        }));
        let resolved = resolve_codex_aggregate_provider(&db, &aggregate, "gpt-5.4")
            .expect("resolve explicit override")
            .expect("mapped responses member");
