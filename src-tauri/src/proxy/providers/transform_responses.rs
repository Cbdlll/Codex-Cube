...
pub(crate) const TOOL_RESULT_ERROR_MARKER: &str = "[codex-cube:tool-result-error]";
...
pub fn anthropic_to_responses(
    ...
    //   （Codex Cube 的 transform 当前是"条件写入"，可能产生缺失）
    // - service_tier: 仅在 FAST mode 开启时写入 "priority"
    //   （与 OpenAI 官方 codex-rs 当前请求结构保持一致）
    // - stream: 必须永远 true（codex-rs 硬编码 true，且 Codex Cube 的
...
mod tests {
    ...
    fn test_anthropic_to_responses_converts_json_string_tool_image() {
        ...
        assert!(serialized.contains("[codex-cube: omitted 20000 bytes]"));
    ...
    fn test_encrypted_reasoning_round_trips_through_anthropic_history() {
        ...
        assert!(thinking["signature"]
            .as_str()
            .is_some_and(|value| value.starts_with("codexcube-openai-reasoning-v1:")));
