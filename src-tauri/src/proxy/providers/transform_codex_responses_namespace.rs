...
//! `422 unknown variant "namespace"`. Codex Cube's Chat/Anthropic transforms
...
const COLLABORATION_TOOLS: &[&str] = &["spawn_agent", "send_message", "followup_task"];

/// Third-party Responses gateways do not understand Codex's private
/// `encrypted: true` schema marker. Removing it from the collaboration
/// message field makes the model emit the task body as ordinary JSON text;
/// the response-side helper below then tells Codex to route that text as a
/// plaintext inter-agent message.
pub(crate) fn prepare_collaboration_message_schemas(body: &mut Value) -> bool {
    ...
    for tool in tools {
        ...
        // MultiAgentV2 emits these as top-level function tools. The encrypted
        // marker is part of the parameter schema, regardless of whether the
        // tool came from a namespace or the V2 registry.
        if tool_type == Some("function")
            && tool_name.is_some_and(|name| COLLABORATION_TOOLS.contains(&name))
        {
                ...
                .and_then(|properties| properties.get_mut("message"))
            {
                if let Some(message_object) = message.as_object_mut() {
                    changed |= message_object.remove("encrypted").is_some();
                }
            }
            continue;
        }
        ...
        if tool.get("type").and_then(Value::as_str) != Some("namespace")
            || tool.get("name").and_then(Value::as_str) != Some(COLLABORATION_NAMESPACE)
        {
            continue;
        }
        let children = if tool.get("tools").is_some() {
            tool.get_mut("tools").and_then(Value::as_array_mut)
        } else {
            tool.get_mut("children").and_then(Value::as_array_mut)
        };
        let Some(children) = children else {
            continue;
        };
        for actual_child in children {
            let Some(name) = actual_child.get("name").and_then(Value::as_str) else {
                continue;
            };
            if !COLLABORATION_TOOLS.contains(&name) {
                continue;
            }
            if let Some(message) = actual_child
                .get_mut("parameters")
                .and_then(Value::as_object_mut)
                .and_then(|parameters| parameters.get_mut("properties"))
                .and_then(Value::as_object_mut)
...
/// Whether the request declares any collaboration tool whose message body is
/// routed through the private encrypted-argument path.
pub(crate) fn has_collaboration_message_tools(body: &Value) -> bool {
    body.get("tools")
        .and_then(Value::as_array)
        .is_some_and(|tools| {
            tools.iter().any(|tool| {
                (tool.get("type").and_then(Value::as_str) == Some("function")
                    && tool
                        ...
                        .is_some_and(|name| COLLABORATION_TOOLS.contains(&name)))
                    || (tool.get("type").and_then(Value::as_str) == Some("namespace")
                        && tool.get("name").and_then(Value::as_str)
                            == Some(COLLABORATION_NAMESPACE)
                        && namespace_children(tool).iter().any(|child| {
...
/// Add the Responses metadata that makes a collaboration function call's
/// arguments a plaintext inter-agent message. Existing non-empty metadata is
/// preserved, since it identifies a genuinely encrypted payload.
pub(crate) fn mark_plaintext_collaboration_calls(value: &mut Value) -> bool {
        ...
        match value {
            ...
            Value::Object(obj) => {
                if obj.get("type").and_then(Value::as_str) == Some("function_call") {
                    ...
                    let is_collaboration = namespace == Some(COLLABORATION_NAMESPACE)
                        && name.is_some_and(|name| COLLABORATION_TOOLS.contains(&name))
                        || namespace.is_none()
                            && name.is_some_and(|name| COLLABORATION_TOOLS.contains(&name))
                        || namespace.is_none()
                            && name.is_some_and(|name| {
                                COLLABORATION_TOOLS.iter().any(|tool| {
                                    name == format!("{COLLABORATION_NAMESPACE}__{tool}")
                                })
                            });
                    if is_collaboration {
                        let existing = obj.get("encrypted_function_args");
                        // Absent or explicitly `null` both mean "no encryption metadata";
                        // Codex treats that as an encrypted call, so normalize to an
                        // empty list to signal plaintext inter-agent arguments.
                        if existing.is_none() || existing.and_then(Value::as_null).is_some() {
                            obj.insert("encrypted_function_args".to_string(), json!([]));
                            changed = true;
                        }
                    }
                }
...
mod tests {
    ...
    fn collaboration_request() -> Value {
        json!({
            ...
            "tools": [{
                ...
                "type": "function", "name": "ordinary_tool", "parameters": {
                    "type": "object", "properties": {
                        "message": { "type": "string", "encrypted": true }
                    }
                }
    ...
    #[test]
    fn collaboration_message_schema_is_plaintext_for_third_party_requests() {
        let mut request = collaboration_request();
        assert!(has_collaboration_message_tools(&request));
        assert!(prepare_collaboration_message_schemas(&mut request));
        assert!(
        ...
        assert_eq!(
            request["tools"][1]["parameters"]["properties"]["message"]["encrypted"],
            json!(true)
        );
    }

    #[test]
    fn top_level_v2_collaboration_message_schema_is_plaintext() {
        ...
        assert!(has_collaboration_message_tools(&request));
        assert!(prepare_collaboration_message_schemas(&mut request));
        assert!(request["tools"][0]["parameters"]["properties"]["message"]
            .get("encrypted")
            .is_none());
        assert!(request["tools"][1]["parameters"]["properties"]["message"]
            .get("encrypted")
            .is_none());
        assert_eq!(
            request["tools"][2]["parameters"]["properties"]["message"]["encrypted"],
            json!(true)
        );
    }

    #[test]
    fn plaintext_spawn_and_followup_calls_get_empty_encryption_metadata() {
        let mut response = json!({
            "output": [
                { "type": "function_call", "namespace": "collaboration", "name": "spawn_agent", "arguments": "{\"message\":\"target file: src/lib.rs\"}" },
                { "type": "function_call", "namespace": "collaboration", "name": "followup_task", "arguments": "{\"message\":\"acceptance: tests pass\"}" },
                { "type": "function_call", "namespace": "collaboration", "name": "send_message", "encrypted_function_args": ["message"], "arguments": "gAAAA" },
                { "type": "function_call", "name": "spawn_agent", "arguments": "{\"message\":\"top-level task\"}" },
                { "type": "function_call", "name": "ordinary_tool", "arguments": "{}" },
                { "type": "function_call", "namespace": "collaboration", "name": "send_message", "encrypted_function_args": null, "arguments": "{\"message\":\"null metadata\"}" }
            ]
        });
        assert!(mark_plaintext_collaboration_calls(&mut response));
        assert_eq!(response["output"][0]["encrypted_function_args"], json!([]));
        assert_eq!(response["output"][1]["encrypted_function_args"], json!([]));
        assert_eq!(
            response["output"][2]["encrypted_function_args"],
            json!(["message"])
        );
        assert_eq!(response["output"][3]["encrypted_function_args"], json!([]));
        assert!(response["output"][4]
            .get("encrypted_function_args")
            .is_none());
        assert_eq!(response["output"][5]["encrypted_function_args"], json!([]));
    }

    #[tokio::test]
    async fn sse_stream_marks_plaintext_collaboration_calls() {
        ...
        assert!(collected.contains("\"encrypted_function_args\":[]"));
    }
