...
/// catalog with a top-level `models` field.  Return the codex-cube–managed model
...
/// codex-cube–owned `model_catalog_json`, using the same path ownership rules as
...
pub async fn handle_models() -> Result<Json<Value>, ProxyError> {
    ...
    let active_catalog_path = match crate::codex_config::read_codex_config_text() {
        Ok(config_text) => {
            crate::codex_config::resolve_codex_cube_catalog_path(&config_text, &config_dir)
        }
        Err(_) => None,
    };
        ...
        active_catalog_path.as_ref().filter(|path| path.exists())
    {
        ...
        if active_catalog_path.is_none() {
            log::debug!(
                "[models] stale guard: catalog not served (model_catalog_json not set to codex-cube catalog)"
            );
        }
        json!({"models": []})
    };
...
pub async fn handle_chat_completions(
        ...
        .await
    {
        ...
        Err(mut err) => {
            ...
            ctx.outbound_model = err.outbound_model.take();
...
async fn handle_responses_for_app(
    ...
    let has_collaboration_message_tools =
        transform_codex_responses_namespace::has_collaboration_message_tools(&body);
    ...
    let restored_bytes = match serde_json::from_slice::<Value>(&body_bytes) {
        Ok(mut value) => {
            if mark_collaboration_plaintext {
                transform_codex_responses_namespace::mark_plaintext_collaboration_calls(&mut value);
            }
...
fn build_codex_proxy_error_response(
    ...
    // 出站模型真值只取 forwarder 实际回填的 `ctx.outbound_model`（请求定稿后记录，
    // 模型解析/回退/改写之后）。绝不能回退到 Provider 配置/默认模型：请求可能
    // 在最终 body 构建或发送前就失败（认证 / base_url 解析等），此时把配置模型标成
    // outbound_model 会伪造 debug 真相。未知时结构化 outbound_model 为 null，
    // error.model 回落为请求模型。
...
fn codex_proxy_error_json(
    ...
    // 实际出站模型与请求模型不同时，两种消息路径（普通错误 / 413）都追加
    // `(上游: ...)` 片段，避免只展示请求模型误导排查。
    let upstream_fragment = outbound_model
    ...
    let message = if upstream_status == Some(413) {
        ...
        // 避免「以为是 Codex-Cube 封装了 nginx / 是本地代理的锅」这种反复出现的误解。
        format!(
            concat!(
                ...
                "provider's server-side limit, not a Codex-Cube limit. ",
                "Provider: {provider}; model: {model}{upstream_fragment}; endpoint: {endpoint}. ",
    ...
    // 结构化字段把「客户端请求的模型」与「实际发往上游的模型」分开暴露：
    // error.model 保持既有键名（向后兼容），但在出站模型已知时指向实际出站
    // 模型——这是上游拒绝时最可操作的模型；request_model / outbound_model
    // 提供完整真相。出站模型未知（请求未发出即失败）时 outbound_model 为
    // null，error.model 回落为 request_model。
    error_obj.insert(
...
fn codex_proxy_error_code(error: &ProxyError) -> &'static str {
    match error {
        ProxyError::ForwardFailed(_) => "codex_cube_forward_failed",
        ProxyError::Timeout(_) | ProxyError::StreamIdleTimeout(_) => "codex_cube_timeout",
        ProxyError::NoAvailableProvider => "codex_cube_no_available_provider",
        ProxyError::AllProvidersCircuitOpen => "codex_cube_all_providers_circuit_open",
        ProxyError::NoProvidersConfigured => "codex_cube_no_providers_configured",
        ProxyError::MaxRetriesExceeded => "codex_cube_max_retries_exceeded",
        ProxyError::ProviderUnhealthy(_) => "codex_cube_provider_unhealthy",
        ProxyError::ConfigError(_) => "codex_cube_config_error",
        ProxyError::TransformError(_) => "codex_cube_transform_error",
        ProxyError::InvalidRequest(_) => "codex_cube_invalid_request",
        ProxyError::AuthError(_) => "codex_cube_auth_error",
        ProxyError::UpstreamError { .. } => "codex_cube_upstream_error",
        ProxyError::DatabaseError(_) => "codex_cube_database_error",
        ProxyError::Internal(_) => "codex_cube_internal_error",
        ProxyError::AlreadyRunning
        | ProxyError::NotRunning
        | ProxyError::BindFailed(_)
        | ProxyError::StopTimeout
        | ProxyError::StopFailed(_)
        | ProxyError::ResponseBodyTooLarge(_) => "codex_cube_proxy_error",
    }
}
...
mod tests {
    ...
    impl TempHome {
        fn new() -> Self {
            ...
            let original_test_home = std::env::var_os("CODEX_CUBE_TEST_HOME");
    ...
    fn build_proxy_state(db: Arc<Database>) -> ProxyState {
        ProxyState {
            ...
            provider_router: Arc::new(crate::proxy::provider_router::ProviderRouter::new(
                db.clone(),
            )),
...
data: {\"type\":\"response.output_item.done\",\"item\":{\"type\":\"message\"}}\n\n";
    ...
    fn codex_proxy_forward_error_includes_context_and_cause() {
        ...
        assert_eq!(body["error"]["code"], "codex_cube_forward_failed");
    ...
    fn codex_proxy_413_points_to_upstream_not_local_proxy() {
        ...
        assert_eq!(body["error"]["request_model"], "gpt-5.5");
        assert!(body["error"]["outbound_model"].is_null());
        assert_eq!(
            body["error"]["model"], "gpt-5.5",
            "unknown outbound model falls back to request model"
        );
        assert_eq!(body["error"]["endpoint"], "/responses");

        // 出站模型已知且与请求模型不同时，413 指引同样标注实际出站模型
        let body = codex_proxy_error_json(
            "HCAI",
            "gpt-5.6-luna",
            "/responses",
        ...
        assert!(message.contains("413"));
        assert!(message.contains("/compact"));
        assert_eq!(body["error"]["request_model"], "gpt-5.6-luna");
        assert_eq!(body["error"]["outbound_model"], "deepseek-v4-flash");
        assert_eq!(body["error"]["model"], "deepseek-v4-flash");
    }

    #[test]
    fn codex_proxy_error_shows_actual_outbound_model_not_configured_model() {
        // 报错信息必须展示「实际发往上游的模型」，而不是 Provider 配置里可能
        // 与实际请求不一致的模型，避免用户 debug 时被误导。结构化字段同时暴露
        // request_model / outbound_model 完整真相，error.model 指向实际出站模型。
        let error = ProxyError::UpstreamError {
            status: 403,
            body: Some("This model is not available in your region.".to_string()),
        };

        // 请求模型与实际出站模型不同（过期会话回退 / 改写）：
        // 消息带 (上游: ...) 且内容为实际出站模型
        let body = codex_proxy_error_json(
            "OpenCode",
            "gpt-5.6-luna",
        ...
        let message = body["error"]["message"].as_str().unwrap();
        assert!(message.contains("model: gpt-5.6-luna (上游: deepseek-v4-flash)"));
        assert!(message.contains("This model is not available in your region."));
        assert_eq!(body["error"]["request_model"], "gpt-5.6-luna");
        assert_eq!(body["error"]["outbound_model"], "deepseek-v4-flash");
        assert_eq!(
            body["error"]["model"], "deepseek-v4-flash",
            "error.model must be the actual outbound model when known"
        );

        // 实际出站模型与请求模型一致（无改写）：不再画蛇添足展示 (上游: ...)
        let body = codex_proxy_error_json(
            "OpenCode",
            "deepseek-v4-flash",
            "/responses",
        ...
        assert_eq!(body["error"]["request_model"], "deepseek-v4-flash");
        assert_eq!(body["error"]["outbound_model"], "deepseek-v4-flash");
        assert_eq!(body["error"]["model"], "deepseek-v4-flash");
    ...
    #[tokio::test]
    #[serial_test::serial]
    async fn build_codex_proxy_error_pre_send_failure_never_claims_configured_model_as_outbound() {
        // 请求在发出前失败（认证错误）：forwarder 从未回填 ctx.outbound_model。
        // 即使 Provider 配置了 deepseek-v4-flash，错误响应也绝不能把配置模型标成
        // outbound_model——否则用户会误以为请求真的以该模型发往了上游。
...
"#
        ...
        crate::settings::set_current_provider(&super::AppType::Codex, Some("deepseek"))
        ...
        assert_eq!(ctx.request_model, "gpt-5.6-luna");
        assert!(
            ctx.outbound_model.is_none(),
            "pre-send failure must leave outbound_model unset"
        );

        let error = ProxyError::AuthError("missing bearer token".to_string());
        let response = super::build_codex_proxy_error_response(&ctx, "/responses", &error)
        ...
        let body_bytes = response
            ...
            .to_bytes();
        let body: Value = serde_json::from_slice(&body_bytes).expect("parse error body");

        assert!(
            body["error"]["outbound_model"].is_null(),
            "provider configured model must not be reported as outbound"
        );
        assert_eq!(body["error"]["request_model"], "gpt-5.6-luna");
        assert_eq!(
            body["error"]["model"], "gpt-5.6-luna",
            "error.model must fall back to request model when no outbound truth exists"
        );
        let message = body["error"]["message"].as_str().unwrap();
        assert!(
            !message.contains("deepseek-v4-flash"),
            "must not claim configured model as actually sent upstream"
        );
        assert!(!message.contains("(上游:"));
    }
}
