...
/// Codex-Cube 代理服务器当前监听的端口
static CODEX_CUBE_PROXY_PORT: OnceCell<RwLock<u16>> = OnceCell::new();

/// 设置 Codex-Cube 代理服务器的监听端口
///
/// 应在代理服务器启动时调用，以便系统代理检测能正确识别自己的端口
pub fn set_proxy_port(port: u16) {
    if let Some(lock) = CODEX_CUBE_PROXY_PORT.get() {
        if let Ok(mut current_port) = lock.write() {
            *current_port = port;
            log::debug!("[GlobalProxy] Updated Codex-Cube proxy port to {port}");
        }
    } else {
        let _ = CODEX_CUBE_PROXY_PORT.set(RwLock::new(port));
        log::debug!("[GlobalProxy] Initialized Codex-Cube proxy port to {port}");
    }
}

/// 获取 Codex-Cube 代理服务器的监听端口
fn get_proxy_port() -> u16 {
    CODEX_CUBE_PROXY_PORT
        .get()
        .and_then(|lock| lock.read().ok())
        .map(|port| *port)
        .unwrap_or(15921) // 默认端口作为回退（与 cc-switch 的 15721 错开）
}
...
fn proxy_points_to_loopback(value: &str) -> bool {
    ...
    // 检查是否指向 Codex-Cube 自己的代理端口
    // 只有指向自己的代理才需要跳过，避免递归
    fn is_codex_cube_proxy_port(port: Option<u16>) -> bool {
        let codex_cube_port = get_proxy_port();
        port == Some(codex_cube_port)
    }

    if let Ok(parsed) = url::Url::parse(value) {
        if let Some(host) = parsed.host_str() {
            // 只有当主机是 loopback 且端口是 Codex-Cube 的端口时才返回 true
            return host_is_loopback(host) && is_codex_cube_proxy_port(parsed.port());
        }
        return false;
    }
...
mod tests {
    ...
    fn test_proxy_points_to_loopback() {
        // 设置 Codex-Cube 代理端口为 15721（默认值）
        set_proxy_port(15721);

        // 只有指向 Codex-Cube 自己端口的 loopback 地址才返回 true
