...
impl ProxyServer {
    ...
    pub async fn set_active_target(&self, app_type: &str, provider_id: &str, provider_name: &str) {
        ...
        self.state
            .provider_router
            .set_runtime_current_provider(app_type, provider_id)
            .await;
    }

    /// Clear the running target when an app leaves proxy takeover.
    pub async fn clear_active_target(&self, app_type: &str) {
        self.state.current_providers.write().await.remove(app_type);
        self.state
            .provider_router
            .clear_runtime_current_provider(app_type)
            .await;
    }

    /// Clear a running target only when it still references a deleted provider.
    pub async fn clear_active_target_if_matches(&self, app_type: &str, provider_id: &str) -> bool {
        let removed = {
            let mut current_providers = self.state.current_providers.write().await;
            let matches = current_providers
                .get(app_type)
                .is_some_and(|(current_id, _)| current_id == provider_id);
            if matches {
