...
/// Codex rows retain app/provider scoping to avoid collisions between upstreams.
...
impl TokenUsage {
    /// 生成稳定 request_id。加入 app/provider 作用域，避免不同上游复用
    /// envelope id 时互相覆盖。
...
mod tests {
    ...
    #[test]
    fn codex_app_scopes_the_session_request_id_namespace() {
