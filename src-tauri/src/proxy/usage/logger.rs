...
mod tests {
    ...
    fn only_session_log_primary_rows_may_be_replaced() -> Result<(), AppError> {
            ...
            for (request_id, data_source) in [
                ...
                conn.execute(
                    "INSERT INTO proxy_request_logs (
                     ...
                     ) VALUES (?1, '_session', 'codex', 'old', 1, 1, 0, 0, 0, 200, 1, ?2)",
        ...
        session_replacement.app_type = "codex".to_string();
    ...
    #[test]
    fn codex_logs_total_input_token_semantics() -> Result<(), AppError> {
        let db = Database::memory()?;
        let logger = UsageLogger::new(&db);
        let log = RequestLog {
            request_id: "codex-semantics".to_string(),
        ...
        let semantics: i64 = conn.query_row(
            "SELECT input_token_semantics FROM proxy_request_logs WHERE request_id = 'codex-semantics'",
            [],
            |row| row.get(0),
        )?;
