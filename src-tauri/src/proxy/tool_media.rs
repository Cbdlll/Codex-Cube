...
pub(crate) const TOOL_RESULT_MEDIA_MOVED_MARKER: &str =
    "[codex-cube: tool result media moved to the following user message]";
pub(crate) const TOOL_RESULT_MEDIA_ATTACHED_MARKER: &str =
    "[codex-cube: tool result media attached as native media]";
...
pub(crate) fn queue_chat_tool_output_media(
    ...
    pending_media.push(json!({
        "type": "text",
        "text": format!("[codex-cube: media output of tool call {call_id}]")
    }));
    ...
    match value {
        Value::String(text) => {
            ...
            if should_omit {
                ...
                *text = format!("[codex-cube: omitted {byte_len} bytes]");
            }
        }
        Value::Array(items) => {
            for item in items {
...
mod tests {
    ...
    fn chat_plan_clamps_residual_base64_inside_json_string_before_serializing() {
        ...
        assert!(plan
            .tool_content
            .contains("[codex-cube: omitted 20000 bytes]"));
    ...
    fn clamp_preserves_long_text_but_removes_data_and_base64_payloads() {
        ...
        assert!(value["data_url"]
            .as_str()
            .unwrap()
            .starts_with("[codex-cube: omitted "));
        assert!(value["raw"]
            .as_str()
            .unwrap()
            .starts_with("[codex-cube: omitted "));
