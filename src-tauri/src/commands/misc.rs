...
pub async fn check_for_updates(handle: AppHandle) -> Result<bool, String> {
    handle
        .opener()
        .open_url(
            "https://github.com/Cbdlll/Codex-Cube/releases/latest",
            None::<String>,
        )
...
fn run_tool_lifecycle_silently(command_line: &str, label: &str) -> Result<(), String> {
    ...
    let bat_file =
        std::env::temp_dir().join(format!("codex_cube_{}_{}.bat", label, std::process::id()));
...
///
/// 判定顺序（命中即返回）：
/// ① Hermes → `<bin_path 绝对> update`;Hermes CLI 自己知道安装环境,避免 Codex Cube
...
fn launch_macos_terminal(config_file: &std::path::Path, cwd: Option<&Path>) -> Result<(), String> {
    ...
    let temp_dir = std::env::temp_dir();
    let script_file = temp_dir.join(format!("codex_cube_launcher_{}.sh", std::process::id()));
...
fn launch_windows_terminal(
    ...
    let bat_file = temp_dir.join(format!("codex_cube_claude_{}.bat", std::process::id()));
...
pub(crate) fn launch_terminal_running(command_line: &str, label: &str) -> Result<(), String> {
    ...
    let (script_file, script_content) = {
        let file = temp_dir.join(format!("codex_cube_{}_{}.sh", label, pid));
        let content = format!(
            r#"#!/usr/bin/env sh
trap 'rm -f "{script_path}"' EXIT
echo "[codex-cube] Starting: {label}"
echo ""
{cmd}
echo ""
echo "[codex-cube] Command exited. Press Enter to close."
read -r _
"#,
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        let bat_file = temp_dir.join(format!("codex_cube_{}_{}.bat", label, pid));
        let content = format!(
            "@echo off\r\necho [codex-cube] Starting: {label}\r\necho.\r\n{cmd}\r\necho.\r\necho [codex-cube] Command exited. Press any key to close.\r\npause >nul\r\ndel \"%~f0\" >nul 2>&1\r\n",
            label = label,
            cmd = command_line,
        );
        std::fs::write(&bat_file, &content).map_err(|e| format!("写入批处理文件失败: {e}"))?;
...
mod tests {
    ...
    #[test]
    fn codex_lifecycle_metadata_is_consistent() {
        let requested = vec!["unsupported".to_string(), "codex".to_string()];
        ...
        assert_eq!(
            npm_install_command_for("codex"),
            Some("npm i -g @openai/codex@latest")
        );
        assert_eq!(official_update_args("codex"), Some("update"));

        for action in [ToolLifecycleAction::Install, ToolLifecycleAction::Update] {
            assert_eq!(
                tool_action_shell_command_for_shell("codex", action, LifecycleCommandShell::Posix)
                    .as_deref(),
                Some("npm i -g @openai/codex@latest")
            );
        }

        // Codex 的静态更新保持包管理器路径（prefers_official_update 不含 codex）。
        assert_eq!(
    ...
    fn resolve_launch_cwd_rejects_missing_directory() {
        let unique = std::time::SystemTime::now()
            ...
            .as_nanos();
        let missing = std::env::temp_dir().join(format!("codex-cube-missing-{unique}"));
    ...
    fn iterm2_applescript_cold_start_avoids_current_window_before_one_exists() {
        let script = build_macos_iterm2_applescript(Path::new("/tmp/codex_cube_launcher.sh"));
    ...
    fn terminal_applescript_cold_start_uses_launch_before_do_script() {
        let script = build_macos_terminal_applescript(Path::new("/tmp/codex_cube_launcher.sh"));
        ...
        assert!(
            script.contains(r#"set launcher_script to "exec sh '/tmp/codex_cube_launcher.sh'""#),
    ...
    fn ghostty_applescript_cold_start_uses_initial_command() {
        let script = build_macos_ghostty_applescript(Path::new("/tmp/codex_cube_launcher.sh"));

        // Warm launches execute through the AppleScript command property, not `open -na ... -e`.
        assert!(
            script.contains(r#"set launcher_command to "sh '/tmp/codex_cube_launcher.sh'""#),
    ...
    fn dash_c_command_wraps_script_path_inside_quoted_arg() {
        ...
        let s = build_macos_dash_c_command(Path::new("/tmp/codex_cube_launcher_1.sh"));
        assert_eq!(s, "exec sh '/tmp/codex_cube_launcher_1.sh'");
