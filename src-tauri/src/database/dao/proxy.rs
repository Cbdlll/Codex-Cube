...
mod tests {
    ...
    async fn test_default_cost_multiplier_round_trip() -> Result<(), AppError> {
        ...
        let default = db.get_default_cost_multiplier("codex").await?;
        assert_eq!(default, "1");

        db.set_default_cost_multiplier("codex", "1.5").await?;
        let updated = db.get_default_cost_multiplier("codex").await?;
    ...
    async fn test_default_cost_multiplier_validation() -> Result<(), AppError> {
        ...
        let err = db
            .set_default_cost_multiplier("codex", "not-a-number")
            .await
            .unwrap_err();
    ...
    async fn test_pricing_model_source_round_trip_and_validation() -> Result<(), AppError> {
        ...
        let default = db.get_pricing_model_source("codex").await?;
        assert_eq!(default, "response");

        db.set_pricing_model_source("codex", "request").await?;
        let updated = db.get_pricing_model_source("codex").await?;
        ...
        let err = db
            .set_default_cost_multiplier("codex", "-0.5")
            .await
            .unwrap_err();
        assert!(matches!(
            err,
