...
//! - 实际文件存储在 ~/.codex-cube/skills/，同步到各应用目录
...
impl Database {
        ...
        let mut stmt = conn
            .prepare(
                "SELECT id, name, description, directory, repo_owner, repo_name, repo_branch,
                        readme_url, enabled_codex, installed_at, content_hash, updated_at
        ...
        let skill_iter = stmt
            .query_map([], |row| {
                Ok(InstalledSkill {
                    ...
                    apps: SkillApps { codex: row.get(8)? },
    ...
    pub fn update_skill_apps(&self, id: &str, apps: &SkillApps) -> Result<bool, AppError> {
        ...
        let affected = conn
            .execute(
                "UPDATE skills SET enabled_codex = ?1 WHERE id = ?2",
                params![apps.codex, id],
            )
            .map_err(|e| AppError::Database(e.to_string()))?;
        Ok(affected > 0)
    }
...
mod tests {
    ...
    fn update_skill_metadata_preserves_enabled_apps() {
        ...
        let mut candidate = skill(&original.id, "updated", SkillApps::only(&AppType::Codex));
