...
const MCP_SERVER_SELECT: &str =
    "SELECT id, name, server_config, description, homepage, docs, tags, enabled_codex FROM mcp_servers";
...
fn row_to_mcp_server(row: &Row<'_>) -> rusqlite::Result<(String, McpServer)> {
    ...
    let enabled_codex: bool = row.get(7)?;
    ...
    pub fn save_mcp_server(&self, server: &McpServer) -> Result<(), AppError> {
        ...
        conn.execute(
            "INSERT OR REPLACE INTO mcp_servers (
                id, name, server_config, description, homepage, docs, tags, enabled_codex
...
mod tests {
    ...
    #[test]
    fn app_flag_update_returns_authoritative_row() {
    ...
    fn concurrent_app_flag_updates_do_not_lose_each_other() {
        ...
        let handles = [AppType::Codex, AppType::Codex].map(|app| {
            let db = Arc::clone(&db);
    ...
    fn app_flag_update_does_not_insert_a_missing_server() {
        ...
        let updated = db
            .update_mcp_server_app_enabled("missing", &AppType::Codex, true)
