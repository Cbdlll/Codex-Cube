...
/// Remove a deleted Codex member from an aggregate provider's persisted state.
///
/// Aggregate mappings are the routing source of truth. Keeping a mapping after its
/// member is deleted lets an obsolete Desktop slot survive in the catalog and can
/// later resolve to a missing provider. The catalog is pruned only when mappings
/// change, so unrelated catalog metadata remains untouched.
fn prune_deleted_codex_aggregate_member(settings: &mut Value, deleted_id: &str) -> bool {
    let Some(root) = settings.as_object_mut() else {
        return false;
    };

    let members_changed = root
        .get_mut("memberProviderIds")
        .and_then(Value::as_array_mut)
        .map(|members| {
            let previous_len = members.len();
            members.retain(|member| member.as_str() != Some(deleted_id));
            members.len() != previous_len
        })
        .unwrap_or(false);

    let aggregate_models_changed = root
        .get_mut(crate::codex_config::CODEX_AGGREGATE_MODELS_KEY)
        .and_then(Value::as_array_mut)
        .map(|models| {
            ...
            models.retain(|model| {
                ...
                member_id != Some(deleted_id)
            });
            models.len() != previous_len
        })
        .unwrap_or(false);

    let mut catalog_changed = false;
    if members_changed || aggregate_models_changed {
        let retained_models: HashSet<String> = root
            .get(crate::codex_config::CODEX_AGGREGATE_MODELS_KEY)
        ...
        if let Some(catalog_models) = root
            .get_mut("modelCatalog")
            .and_then(Value::as_object_mut)
            .and_then(|catalog| catalog.get_mut("models"))
            .and_then(Value::as_array_mut)
        {
            let previous_len = catalog_models.len();
            catalog_models.retain(|model| {
                let Some(slug) = model
                    .get("model")
                ...
                retained_models.contains(slug)
            });
            catalog_changed = catalog_models.len() != previous_len;
        }
    }
...
impl Database {
    ...
    /// Delete a Codex provider together with every aggregate reference to it.
    ///
    /// Updating aggregates and deleting the member happen in one transaction, so
    /// a failed delete cannot leave a partially cleaned routing table behind.
    /// Returns the aggregate provider IDs whose persisted configuration changed.
    pub fn delete_codex_provider_and_prune_aggregate_references(
        &self,
        id: &str,
        ...
        let mut changed_aggregate_ids = Vec::new();
        for (aggregate_id, settings_json, meta_json) in aggregate_rows {
            let meta: ProviderMeta = serde_json::from_str(&meta_json).unwrap_or_default();
            if meta.provider_type.as_deref() != Some("aggregate") {
                continue;
            }

            let mut settings: Value = serde_json::from_str(&settings_json).unwrap_or(Value::Null);
            if !prune_deleted_codex_aggregate_member(&mut settings, id) {
                continue;
            }

            let serialized_settings = serde_json::to_string(&settings).map_err(|e| {
                AppError::Database(format!("Failed to serialize settings_config: {e}"))
            })?;
            tx.execute(
                "UPDATE providers
                 SET settings_config = ?1
                 WHERE id = ?2 AND app_type = 'codex'",
                params![serialized_settings, aggregate_id],
            )
            .map_err(|e| AppError::Database(e.to_string()))?;
            changed_aggregate_ids.push(aggregate_id);
        }
...
mod ensure_official_seed_tests {
    ...
    #[test]
    fn ensure_inserts_codex_official_when_missing() {
