...
/// 导出/导入使用的 SQL 备份文件头。
const CODEX_CUBE_SQL_EXPORT_HEADER: &str = "-- Codex-Cube SQLite 导出";
...
impl Database {
    ...
    fn validate_sql_export(sql: &str) -> Result<(), AppError> {
        ...
        if trimmed.starts_with(CODEX_CUBE_SQL_EXPORT_HEADER) {
            return Ok(());
        }

        Err(AppError::localized(
            "backup.sql.invalid_format",
            "仅支持导入由 Codex-Cube 导出的 SQL 备份文件。",
            "Only SQL backups exported by Codex-Cube are supported.",
        ))
    }
    ...
    pub(crate) fn backup_database_file(&self) -> Result<Option<PathBuf>, AppError> {
        let db_path = get_app_config_dir().join("codex-cube.db");
        if !db_path.exists() {
            return Ok(None);
        }
...
mod tests {
    ...
    impl TestHomeGuard {
        fn new() -> Self {
            ...
            let previous_test_home = std::env::var_os("CODEX_CUBE_TEST_HOME");
            std::env::set_var("CODEX_CUBE_TEST_HOME", temp_dir.path());
            ...
            // CODEX_CUBE_TEST_HOME and makes import exercise its safety backup.
            let config_dir = temp_dir.path().join(".codex-cube");
            std::fs::create_dir_all(&config_dir).expect("create isolated config directory");
            std::fs::File::create(config_dir.join("codex-cube.db"))
    ...
    impl Drop for TestHomeGuard {
        fn drop(&mut self) {
            match self.previous_test_home.as_ref() {
                Some(previous) => std::env::set_var("CODEX_CUBE_TEST_HOME", previous),
    ...
    fn import_rejects_cross_file_statements_and_leaves_no_file_behind() -> Result<(), AppError> {
        ...
        for (label, template) in cases {
            let target = test_home
                .path()
                .join(format!("codex-cube-authorizer-{label}.sqlite"));

            // 合法的导出头 + 越界语句。头部校验只比前缀，这份输入过得了它，
            // 真正拦下来的必须是 authorizer。
            let malicious = format!(
                "{}\n{}\n",
                super::CODEX_CUBE_SQL_EXPORT_HEADER,
