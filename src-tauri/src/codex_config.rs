...
pub const CODEX_MODEL_PROVIDER_ID: &str = "custom";
/// Temporary model-provider id used while the built-in `codex-official`
/// provider is routed through Codex-Cube.  A dedicated id is an ownership
...
pub const CODEX_OFFICIAL_PROXY_PROVIDER_ID: &str = "codex-cube-official";

pub fn is_codex_official_proxy_id(id: &str) -> bool {
    id == CODEX_OFFICIAL_PROXY_PROVIDER_ID
}
pub const CODEX_MODEL_CATALOG_FILENAME: &str = "codex-cube-model-catalog.json";
...
/// this per the vendors' official Codex docs. Also doubles as Codex Cube's
...
///
/// - `ProxyChat`: Codex Cube's proxy takes over and converts Responses<->Chat,
...
pub enum CodexCatalogToolProfile {
    ProxyChat,
    NativeResponses,
    /// Codex talks (through Codex Cube's proxy) to a native Anthropic Messages
...
pub fn get_codex_model_catalog_path() -> PathBuf {
    get_codex_config_dir().join(CODEX_MODEL_CATALOG_FILENAME)
}
...
pub fn write_codex_live_atomic(
    ...
    let current_config = old_config
        .as_deref()
        .map(String::from_utf8_lossy)
        .unwrap_or_default();
    let cfg_text = crate::codex_subagents::preserve_managed_providers_for_live_write(
        &current_config,
        &cfg_text,
    )?;
...
fn codex_catalog_model_entry(
    ...
    if profile != CodexCatalogToolProfile::ProxyChat {
        ...
        // Codex's model-switch instructions come from
        // `model_messages.instructions_template`; without it `get_model_instructions()`
        // is empty and Codex never emits the model-switch developer message. The clean
        // native template has no `model_messages`, so inject a NEUTRAL one — no gpt-5.5
        // persona text, no personality messages — purely to announce model switches.
        entry_obj.insert(
            "model_messages".to_string(),
            json!({ "instructions_template": "Continue the conversation as Codex." }),
        );
        entry_obj.insert("shell_type".to_string(), json!("shell_command"));
        // Native `/responses` gateways that do not publish an official Codex
        // catalog (aggregate providers, generic relays) must still advertise
        // multi-agent support, otherwise Codex never exposes the collaboration
        // tools (`spawn_agent` / `send_message` / `wait_agent` / ...) and the
        // session falls back to local agent search. `v2` is the value Codex
        // honors for these tools; official vendor catalogs carry their own
        // declaration and bypass this function.
...
fn codex_catalog_model_specs(settings: &Value) -> Vec<CodexCatalogModelSpec> {
    let model_catalog_models = settings
        .get("modelCatalog")
        .and_then(|catalog| catalog.get("models"))
        .and_then(|models| models.as_array())
        .filter(|models| !models.is_empty());
    let aggregate_models = settings
        .get(CODEX_AGGREGATE_MODELS_KEY)
        .and_then(|models| models.as_array())
        .filter(|models| !models.is_empty());
    let Some(models) = model_catalog_models.or(aggregate_models) else {
        return Vec::new();
    };
    ...
    specs
        ...
        .map(|mut spec| {
            if spec.context_window.is_none() {
                spec.context_window = Some(codex_default_context_window_for_model(
                    &spec.model,
                    config_text,
                ));
            }
...
/// catalog (models.json) that Codex Cube mirrors verbatim. Matched against
...
/// rejection bug.
const CODEX_DEEPSEEK_OFFICIAL_CATALOG_HOSTS: &[&str] = &["deepseek.com", "opencode.ai"];

/// Bundled copy of DeepSeek's official Codex models.json. The vendor catalog
/// remains verbatim while it is used as a template; generated Codex catalogs
/// and the Desktop `models_cache.json` projection are normalized to six GPT
/// reasoning levels before they are written.
/// Keeps the official capability profile (freeform apply_patch, GPT-5 harness
/// base_instructions, web_search supported, 1m context) while ensuring the
/// Desktop picker does not collapse the reasoning menu to a single level.
...
/// runs through Codex Cube's converter (gpt-5.5 template contract) and the
...
fn codex_model_catalog_from_settings(
    ...
    let specs = codex_catalog_model_specs_with_default_context(specs, config_text);
...
fn set_codex_model_catalog_json_field(
    ...
    match catalog_path {
        Some(catalog_path) => {
            // Only claim the pointer when it is absent or already codex-cube-owned.
            ...
            // `resolve_codex_cube_catalog_path` relies on.
            let is_codex_cube_owned = doc
                .get("model_catalog_json")
                .and_then(|item| item.as_str())
                .map(|path| {
                    Path::new(path).file_name().and_then(|name| name.to_str())
                        == Some(CODEX_MODEL_CATALOG_FILENAME)
                })
                .unwrap_or(true);
            if is_codex_cube_owned {
                // Codex Desktop's app-server does not guarantee that its
                // working directory is `~/.codex`, so a bare filename is not
                // reliably resolvable there. Always persist the absolute
                // path owned by Codex Cube; this keeps model/list metadata
                // available to Desktop and prevents custom-model fallback.
                let absolute_path = if catalog_path.is_absolute() {
                    catalog_path.to_path_buf()
                } else {
                    std::env::current_dir()
                        .map_err(|error| {
                            AppError::Message(format!("无法解析 Codex 模型目录绝对路径: {error}"))
                        })?
                        .join(catalog_path)
                };
                doc["model_catalog_json"] =
                    toml_edit::value(absolute_path.to_string_lossy().into_owned());
            }
        }
        None => {
...
/// but only when it carries Codex Cube's own `"disabled"` sentinel, so switching
...
/// lifecycle is bound to the codex-cube catalog pointer so the field is set/cleaned
...
fn prepare_codex_config_text_with_model_catalog_at(
    settings: &Value,
    config_text: &str,
    profile: CodexCatalogToolProfile,
    catalog_path: &Path,
) -> Result<String, AppError> {
    if let Some(mut catalog) = codex_model_catalog_from_settings(settings, config_text, profile)? {
        if let Some(models) = catalog.get_mut("models") {
            normalize_codex_reasoning_levels(models);
        }
...
/// codex-cube–maintained catalog file referenced by `~/.codex/config.toml` and
...
/// codex-cube–generated file (identified by filename
/// `codex-cube-model-catalog.json`). A user-managed external catalog file is
...
pub fn read_codex_model_catalog_simplified_from_live() -> Result<Option<Value>, AppError> {
    ...
    let Some(catalog_path) = resolve_codex_cube_catalog_path(&config_text, &config_dir) else {
        return Ok(None);
    };
...
/// Read the Codex Cube model catalog file with a size cap.
...
/// Given `config.toml` text, resolve the on-disk path of the codex-cube–owned
...
pub(crate) fn resolve_codex_cube_catalog_path(
    config_text: &str,
    base_dir: &Path,
) -> Option<PathBuf> {
    ...
    let is_codex_cube_owned = referenced_path.file_name().and_then(|name| name.to_str())
        == Some(CODEX_MODEL_CATALOG_FILENAME);
    if !is_codex_cube_owned {
        return None;
    }
    ...
    // `set_catalog_json_none_removes_codex_cube_owned_by_filename` 的场景注释）。
    ...
    // /etc）能让 `link/codex-cube-model-catalog.json` 通过上面的检查，读取却
...
pub(crate) fn codex_settings_has_inline_model_catalog_source(settings: &Value) -> bool {
    settings.get("modelCatalog").is_some() || !codex_aggregate_model_entries(settings).is_empty()
}

/// 是否有实际可生成的模型目录条目（非空 `modelCatalog.models` 或非空
/// `aggregateModels`）。
///
/// 与 [`codex_settings_has_inline_model_catalog_source`] 的区别：仅当目录里真的
/// 有模型时才重建 `models_cache.json`。空目录（如普通供应商被 `attach_*` 注入
/// 空 `modelCatalog` 用于清掉旧指针）必须走恢复/清除路径，否则聚合 Provider 的
/// Desktop 槽位缓存会残留在普通供应商上。
pub(crate) fn codex_settings_has_generated_models(settings: &Value) -> bool {
    let has_catalog_models = settings
        .get("modelCatalog")
        .and_then(|catalog| catalog.get("models"))
        .and_then(Value::as_array)
        .is_some_and(|models| !models.is_empty());
    has_catalog_models || !codex_aggregate_model_entries(settings).is_empty()
}
...
pub fn write_codex_provider_live_with_catalog(
    ...
    // 桌面端模型列表来自 models_cache.json，不依赖官方登录态。任何非官方
    // Provider（普通自定义或聚合）只要带 modelCatalog，都同步生成桌面端缓存。
    // 缓存可再生成：失败只记日志，不能把已提交的切换报成失败。
    if let Err(e) = sync_codex_models_cache_for_provider(category, settings) {
        log::warn!("同步 Codex models_cache 失败（可再生成，不影响切换）: {e}");
    }
    Ok(())
}
...
pub fn apply_codex_official_proxy_route(
    ...
    // Clean only Codex-Cube's placeholder from every stale provider table. Real
...
/// Whether a live Codex config is the official route projected by Codex-Cube.
pub fn codex_config_has_official_proxy_route(config_text: &str) -> bool {
    if !config_text.contains(CODEX_OFFICIAL_PROXY_PROVIDER_ID) {
        return false;
    }
    config_text
        .parse::<DocumentMut>()
        ...
        .map(|id| is_codex_official_proxy_id(&id))
        .unwrap_or(false)
}

/// Remove only the official takeover route owned by Codex-Cube. This is a
...
pub fn remove_codex_official_proxy_route(config_text: &str) -> Result<String, AppError> {
    ...
    match doc.get("model_provider").and_then(|item| item.as_str()) {
        Some(id) if is_codex_official_proxy_id(id) => {}
    ...
    if let Some(item) = doc.as_table_mut().remove("model_providers") {
        ...
        providers.remove(CODEX_OFFICIAL_PROXY_PROVIDER_ID);
        remove_codex_proxy_placeholders_from_providers(&mut providers);
...
/// Remove a provider's `[model_providers.<provider_id>]` section from a Codex
/// `config.toml` (standard or inline table). When that provider is the active
/// `model_provider`, also scrub the routing fields it owned: top-level
/// `model_provider` / `model`, the top-level `base_url` only when it matches
/// `provider_base_url` (trailing `/` ignored), `experimental_bearer_token`,
/// and the codex-cube-owned `model_catalog_json` pointer. All other sections
/// and user content are preserved.
pub fn scrub_codex_provider_from_config(
    config_text: &str,
    provider_id: &str,
    ...
    let providers_empty = doc
        ...
        .map(|providers| {
            had_provider = providers.remove(provider_id).is_some();
            providers.is_empty()
        })
        .unwrap_or(false);
    if providers_empty {
    ...
    if doc.get("model_provider").and_then(|item| item.as_str()) != Some(provider_id) {
        // 非当前 provider：只清理它的残留 section；没有任何实际变化时原样
        // 返回，避免无谓地重新序列化用户配置。
        return Ok(if had_provider {
            doc.to_string()
        } else {
    ...
    if let Some(provider_base_url) = provider_base_url {
        let expected = provider_base_url.trim_end_matches('/');
        let base_url_matches = doc
            .get("base_url")
            .and_then(|item| item.as_str())
            .map(|value| value.trim_end_matches('/') == expected)
            .unwrap_or(false);
        if base_url_matches {
    ...
    // Only an active provider owns the codex-cube catalog pointer; a non-active
    // provider never set it, so leave the field (and any user-owned catalog)
    // untouched.
    let scrubbed = doc.to_string();
    set_codex_model_catalog_json_field(&scrubbed, None)
}
...
pub fn inject_codex_unified_session_bucket(config_text: &str) -> Result<String, AppError> {
    ...
    let existing_custom_conflicts = doc
        .get("model_providers")
        .and_then(|item| item.as_table())
        .and_then(|providers| providers.get(CODEX_MODEL_PROVIDER_ID))
        .and_then(|item| item.as_table())
    ...
    doc["model_provider"] = toml_edit::value(CODEX_MODEL_PROVIDER_ID);
    ...
    if let Some(providers) = doc["model_providers"].as_table_mut() {
        if !providers.contains_key(CODEX_MODEL_PROVIDER_ID) {
            providers.insert(
...
pub fn strip_codex_unified_session_bucket(config_text: &str) -> Result<String, AppError> {
    ...
    if doc.get("model_provider").and_then(|item| item.as_str()) != Some(CODEX_MODEL_PROVIDER_ID) {
        return Ok(config_text.to_string());
    }
    ...
    let providers_empty = doc["model_providers"]
        .as_table_mut()
        .map(|providers| {
            providers.remove(CODEX_MODEL_PROVIDER_ID);
            providers.is_empty()
        })
        .unwrap_or(false);
    if providers_empty {
...
/// 聚合 Provider 接管时生成 `config.toml` 文本：固定 `model_provider = "custom"`
/// 并建立指向本地代理的 `[model_providers.custom]` 段。
///
/// 聚合 Provider 的 `settings_config.config` 是空的，直接沿用
/// `update_codex_toml_field` 只会得到顶层 `base_url`，缺少 `model_provider`
/// 会让 Codex 回落到 api.openai.com 官方端点（用 auth.json 里残留的 key 请求
/// 而得到 401），根本到不了 Codex-Cube 代理。这里以现有 config.toml 为基底，
/// 只投影路由字段：既保证请求先到本地代理再按模型路由，也保留用户在 Codex
/// 里已有的设置（语言、插件、MCP、features 等），否则每次重启 Codex-Cube
/// 都会把这些设置重置为默认。
pub fn build_aggregate_takeover_toml(
    ...
    custom["name"] = toml_edit::value(provider.name.as_str());
    custom["base_url"] = toml_edit::value(proxy_url.trim_end_matches('/'));
    ...
    {
        // 替换旧的 custom 段（残留 base_url / bearer token 等），其它用户
        // 自定义 model_providers 段保留。
        model_providers.remove("custom");
        model_providers.insert("custom", toml_edit::Item::Table(custom));
    } else {
        // model_providers 缺失或是 inline table 等无法安全合并的形状：
        // 重建只含 custom 的表（与旧行为一致）。
        let mut model_providers = toml_edit::Table::new();
        model_providers.insert("custom", toml_edit::Item::Table(custom));
        doc["model_providers"] = toml_edit::Item::Table(model_providers);
    }
...
const CODEX_TAKEOVER_PROJECTED_KEYS: &[&str] = &[
    "model",
    "model_provider",
    "base_url",
    "wire_api",
...
const CODEX_LIVE_PERMISSION_KEYS: &[&str] = &[
    "default_permissions",
    "permissions",
    "sandbox_mode",
...
/// Copy only Codex permission state from the current Live config into a target
/// provider snapshot before writing it. Model/provider/catalog fields remain
/// isolated per provider.
pub fn merge_codex_live_permission_settings(target: &Value, live: &Value) -> Value {
    let Some(live_cfg) = live.get("config").and_then(Value::as_str) else {
        return target.clone();
    };
    let Ok(live_doc) = live_cfg.parse::<DocumentMut>() else {
        return target.clone();
    };
    let mut target_doc = match target.get("config") {
        Some(Value::String(target_cfg)) => {
            let Ok(doc) = target_cfg.parse::<DocumentMut>() else {
                return target.clone();
            };
            doc
        }
        None | Some(Value::Null) if target.is_object() => DocumentMut::new(),
        _ => return target.clone(),
    };

    if !CODEX_LIVE_PERMISSION_KEYS
        .iter()
        .any(|key| live_doc.get(key).is_some())
    {
        return target.clone();
    }

    // Replace the complete permission namespace. Otherwise a stale provider
    // snapshot can revive an old read-only/default profile or combine the new
    // `default_permissions` key with the legacy `sandbox_mode` syntax.
    let mut changed = false;
    for key in CODEX_LIVE_PERMISSION_KEYS {
        changed |= target_doc.as_table_mut().remove(key).is_some();
    }
    for key in CODEX_LIVE_PERMISSION_KEYS {
        let Some(value) = live_doc.get(key) else {
            continue;
        };
        target_doc.insert(key, value.clone());
        changed = true;
    }
    if !changed {
        return target.clone();
...
pub fn merge_codex_live_user_settings_into_backup(backup: &Value, live: &Value) -> Value {
    ...
    let Ok(mut merged) = backup_cfg.parse::<DocumentMut>() else {
        return backup.clone();
    };
    // 只有 Live 确实带着本地代理投影时才合并，避免无关 Live 内容污染备份。
    if !codex_config_text_has_proxy_projection(&live_doc) {
        return backup.clone();
    }
    for (key, value) in live_doc.iter() {
        if CODEX_TAKEOVER_PROJECTED_KEYS.contains(&key) {
            continue;
        }
...
/// 判断 Codex config.toml 文档是否带 Codex-Cube 本地代理投影。
fn codex_config_text_has_proxy_projection(doc: &DocumentMut) -> bool {
    if doc
        .get("experimental_bearer_token")
        .and_then(|item| item.as_str())
        == Some(CODEX_PROXY_AUTH_PLACEHOLDER)
    ...
    provider_table
        .get("base_url")
        .and_then(|item| item.as_str())
        .is_some_and(|base_url| base_url.contains("127.0.0.1") || base_url.contains("localhost"))
}

/// 从 `codex --version` 输出解析「主版本号整段」（如 `0.147.0`）。
///
/// Codex 版本形如 `0.147.0-alpha.6.5` / `0.147.0`，Codex 桌面端要求
/// `models_cache.json.client_version` 与 models-manager 的整段版本完全一致，
/// 因此取前三个纯数字段，丢弃预发布/构建后缀。
fn parse_codex_version_whole(output: &str) -> Option<String> {
    for token in output.split_whitespace() {
        let mut parts = Vec::new();
        let mut valid = true;
        for part in token.split('.') {
            let numeric = part.split(['-', '+']).next().unwrap_or("");
...
fn codex_version_from_version_json(value: &Value) -> Option<String> {
    ["latest_version", "current_version", "version"]
        .into_iter()
        .find_map(|key| value.get(key).and_then(Value::as_str))
        .and_then(parse_codex_version_whole)
}
...
fn run_codex_version_with_timeout(candidate: &Path) -> Option<std::process::Output> {
    let mut child = codex_version_command(candidate).spawn().ok()?;
        ...
        match child.try_wait() {
            ...
            Ok(None) => {
                if std::time::Instant::now() >= deadline {
                    let _ = child.kill();
                    let _ = child.wait();
                    log::warn!("`{} --version` 超时，跳过", candidate.display());
                    return None;
                }
                std::thread::sleep(std::time::Duration::from_millis(50));
            }
            Err(err) => {
                log::debug!("等待 `{} --version` 失败: {err}", candidate.display());
                return None;
            }
        }
    }
...
/// 探测本机 Codex CLI 的整段版本（`models_cache.json.client_version` 用）。
///
/// 遍历 `codex_cli_candidates()` 与 macOS 官方应用内置 CLI，取第一个能
/// 解析出版本的结果；结果按进程缓存（测试绕过全局缓存）。
#[cfg(not(test))]
static CODEX_CLIENT_VERSION_CACHE: OnceCell<Option<String>> = OnceCell::new();

fn detect_codex_client_version_whole() -> Option<String> {
    #[cfg(not(test))]
    {
        if let Some(cached) = CODEX_CLIENT_VERSION_CACHE.get() {
            return cached.clone();
        }
    }
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        if bundled.exists() && !candidates.contains(&bundled) {
    ...
    for candidate in candidates {
        ...
        let output = match run_codex_version_with_timeout(&candidate) {
            Some(output) if output.status.success() => output,
            Some(_) => {
                log::debug!("`{label} --version` failed");
                continue;
            }
            None => {
                log::debug!("failed to run `{label} --version`");
                continue;
            }
        };
        let text = String::from_utf8_lossy(&output.stdout);
        if let Some(version) = parse_codex_version_whole(&text) {
            log::debug!("detected codex client version {version} from {label}");
            detected = Some(version);
            break;
        }
    }

    // Codex Desktop writes this file even when a GUI-launched process cannot
    // execute the CLI (minimal PATH, launch-service restrictions, or a transient
    // executable update). Its version uses the same major.minor.patch identity
    // required by models_cache.json.
    if detected.is_none() {
        let version_path = get_codex_config_dir().join("version.json");
        detected = fs::read_to_string(&version_path)
            .ok()
            .and_then(|text| serde_json::from_str::<Value>(&text).ok())
            .and_then(|value| codex_version_from_version_json(&value));
        if let Some(version) = detected.as_deref() {
            log::debug!(
                "detected codex client version {version} from {}",
    ...
    #[cfg(not(test))]
    {
        if let Some(version) = detected.clone() {
            let _ = CODEX_CLIENT_VERSION_CACHE.set(Some(version));
        }
    }
    detected
}
...
/// 聚合 Provider 的 `settingsConfig` 中自定义模型列表的键。
pub(crate) const CODEX_AGGREGATE_MODELS_KEY: &str = "aggregateModels";
/// Codex Cube 写入 `models_cache.json` 的 etag 前缀（聚合 Provider）。
const CODEX_AGGREGATE_CACHE_ETAG_PREFIX: &str = "W/\"codex-cube-";
/// 覆盖 Codex 桌面端 `models_cache.json` 前的原始缓存备份文件名。
const CODEX_AGGREGATE_CACHE_BACKUP_FILENAME: &str = "models_cache.codex-cube-backup.json";
/// Reasoning effort values exposed by the GPT model picker. Every generated
...
fn normalize_codex_reasoning_levels(models: &mut Value) {
    ...
    for model in model_list {
        ...
        let existing: Vec<(String, String)> = levels
            .drain(..)
            .filter_map(|level| {
                let effort = level.get("effort").and_then(Value::as_str)?;
                if !DESKTOP_REASONING_EFFORTS.contains(&effort) {
                    return None;
                }
                let description = level
                    .get("description")
                    .and_then(Value::as_str)
                    .unwrap_or(desktop_reasoning_description(effort))
                    .to_string();
                Some((effort.to_string(), description))
            })
            .collect();
        ...
        *levels = DESKTOP_REASONING_EFFORTS
            ...
            .map(|effort| {
                let description = existing
                    ...
                    .unwrap_or_else(|| desktop_reasoning_description(effort).to_string());
...
fn desktop_reasoning_description(effort: &str) -> &'static str {
    match effort {
...
/// 聚合 Provider 的一条模型映射：对外插槽名 -> 成员供应商 + 上游模型。
#[derive(Debug, Clone, PartialEq, Eq)]
pub(crate) struct CodexAggregateModelEntry {
    pub model: String,
...
/// 解析聚合 Provider `settingsConfig.aggregateModels` 的模型映射列表。
///
/// 兼容 snake_case（`provider_id` / `upstream_model`）与 camelCase
/// （`providerId` / `upstreamModel`）两种存量形态。
pub(crate) fn codex_aggregate_model_entries(settings: &Value) -> Vec<CodexAggregateModelEntry> {
    ...
    models
        ...
        .filter_map(|item| {
            ...
            let api_format = item
                .get("apiFormat")
                .or_else(|| item.get("api_format"))
                .or_else(|| item.get("wireApi"))
                .or_else(|| item.get("wire_api"))
                .and_then(Value::as_str)
                .map(str::trim)
                .filter(|format| !format.is_empty())
                .map(str::to_string);
            Some(CodexAggregateModelEntry {
                model,
                provider_id,
                upstream_model,
                api_format,
...
/// `models_cache.json` 是否为 Codex Cube 自己写入的（聚合 Provider 或官方缓存）。
fn codex_models_cache_is_codex_cube_owned(cache: &Value) -> bool {
    cache
        .get("etag")
        .and_then(Value::as_str)
        .is_some_and(|etag| etag.starts_with(CODEX_AGGREGATE_CACHE_ETAG_PREFIX))
}

/// `models_cache.json` 里由用户在桌面端调整、且 Codex-Cube 不应覆盖的字段。
///
/// Codex-Cube 每次接管都会按 Provider 目录重建 `models_cache.json`，如果不
/// 保留这些字段，用户设置的推理档位（max 等）、推理摘要和默认语气会在重启
/// 后被重置为默认值。
fn preserve_codex_models_cache_user_preferences(models: &mut Value, existing: &Value) {
    let Some(existing_models) = existing.get("models").and_then(Value::as_array) else {
        return;
    };
    let mut by_slug: HashMap<&str, &Value> = HashMap::new();
    for entry in existing_models {
        if let Some(slug) = entry.get("slug").and_then(Value::as_str) {
            by_slug.insert(slug, entry);
        }
    }
    let Some(generated) = models.as_array_mut() else {
        return;
    };
    for model in generated.iter_mut() {
        let Some(slug) = model.get("slug").and_then(Value::as_str) else {
            continue;
        };
        let Some(existing_entry) = by_slug.get(slug) else {
            continue;
        };
        let Some(obj) = model.as_object_mut() else {
            continue;
        };
        let Some(existing_obj) = existing_entry.as_object() else {
            continue;
        };
        for (key, value) in existing_obj {
            let is_user_preference = matches!(
                key.as_str(),
                "default_reasoning_level" | "default_reasoning_summary"
            ) || key.to_ascii_lowercase().contains("tone");
...
/// 非官方 Provider 激活时重建 `models_cache.json`（Codex 桌面端模型列表来源）。
///
/// - 首次写入前把非 Codex Cube 的原始缓存备份到
///   `models_cache.codex-cube-backup.json`，保证关闭接管时能原样恢复。
/// - 目录条目复用现有 `modelCatalog` 投影（`codex_catalog_model_entry`），
///   与终端 `model_catalog_json` 保持一致。
pub fn write_codex_models_cache_for_provider(
    codex_dir: &Path,
    settings: &Value,
) -> Result<(), AppError> {
    let config_text = read_codex_config_text()?;
    write_codex_models_cache_for_provider_at(codex_dir, settings, &config_text, None)
}

/// 同 [`write_codex_models_cache_for_provider`]，但配置文本由调用方传入
/// （便于测试隔离，不读取真实 CODEX_HOME）。
///
/// `client_version` 为 `None` 时自动探测本机 Codex CLI 版本；探测失败则
/// 只恢复/清除已有 Codex Cube 缓存，绝不写入缺少 `client_version` 的
/// 无效缓存（Codex 桌面端会整体拒绝）。
fn write_codex_models_cache_for_provider_at(
    ...
    let mut models = catalog.get("models").cloned().unwrap_or_else(|| json!([]));
    normalize_codex_reasoning_levels(&mut models);

    let live_path = codex_dir.join("models_cache.json");
    let existing: Value = fs::read_to_string(&live_path)
        .ok()
        .and_then(|text| serde_json::from_str(&text).ok())
        .unwrap_or_else(|| json!({}));

    if !codex_models_cache_is_codex_cube_owned(&existing) && live_path.exists() {
        let backup_path = codex_dir.join(CODEX_AGGREGATE_CACHE_BACKUP_FILENAME);
        fs::copy(&live_path, &backup_path).map_err(|e| AppError::io(&live_path, e))?;
    }

    // 桌面端用户偏好（推理档位 / 推理摘要 / 默认语气）随模型目录一起存缓存。
    // 重建前先从现有缓存按 slug 带过来，否则每次重启都会被重置为默认值。
    preserve_codex_models_cache_user_preferences(&mut models, &existing);

    let version_owned = match client_version {
        Some(version) if !version.trim().is_empty() => Some(version.trim().to_string()),
        Some(_) => None,
        None => detect_codex_client_version_whole(),
    };
    let Some(client_version) = version_owned.as_deref() else {
        log::warn!(
            "[codex] cannot detect codex client version for models_cache.json; restoring/clearing cache"
        );
        restore_or_clear_codex_models_cache(codex_dir)?;
        return Ok(());
    };

    let now = chrono::Utc::now();
    // 模型列表与用户偏好都未变化时沿用原 etag / fetched_at，避免桌面端
    // 认为缓存刷新而把用户设置重新初始化（推理档位、语气等）。
    let models_unchanged = codex_models_cache_is_codex_cube_owned(&existing)
        && existing.get("models") == Some(&models);
    let (etag, fetched_at) = if models_unchanged {
        (
            existing
                .get("etag")
                .and_then(Value::as_str)
        ...
        (
            format!("W/\"codex-cube-aggregate-{}\"", now.timestamp()),
    ...
    crate::config::write_json_file(&live_path, &cache)?;
    log::info!("[codex] models_cache.json refreshed for provider model catalog");
    Ok(())
}

/// 按当前 Codex Provider 同步桌面端 `models_cache.json`。
///
/// 官方 Provider 或没有 modelCatalog 的供应商恢复/清除 Codex Cube 写入的
/// 缓存；其余非官方 Provider 重建缓存，保证不登录 OpenAI 官方账号也能在
/// 桌面模型选择器中看到第三方模型。
pub(crate) fn sync_codex_models_cache_for_provider(
    category: Option<&str>,
    settings: &Value,
) -> Result<(), AppError> {
    let codex_dir = get_codex_config_dir();
    if category == Some("official") || !codex_settings_has_generated_models(settings) {
        restore_or_clear_codex_models_cache(&codex_dir)
    } else {
        write_codex_models_cache_for_provider(&codex_dir, settings)
    }
}

/// 关闭 Codex 接管或切离带模型目录的 Provider 时恢复/清除 `models_cache.json`。
///
/// 仅当缓存是 Codex Cube 写入的才处理（幂等）：有备份则恢复备份，否则删除
/// Codex Cube 缓存，让 Codex 桌面端重新拉取真实目录。
pub fn restore_or_clear_codex_models_cache(codex_dir: &Path) -> Result<(), AppError> {
    let live_path = codex_dir.join("models_cache.json");
    let existing: Value = fs::read_to_string(&live_path)
        .ok()
        .and_then(|text| serde_json::from_str(&text).ok())
    ...
    if !codex_models_cache_is_codex_cube_owned(&existing) {
        return Ok(());
    }

    let backup_path = codex_dir.join(CODEX_AGGREGATE_CACHE_BACKUP_FILENAME);
    if backup_path.exists() {
        // 恢复原缓存时把用户在当前 Codex Cube 缓存里设置的偏好（推理档位 /
        // 推理摘要 / 默认语气）按 slug 带回去，避免重启后桌面端重新初始化为默认。
        let mut restored: Value = match fs::read_to_string(&backup_path) {
            Ok(text) => serde_json::from_str(&text).map_err(|e| AppError::json(&backup_path, e))?,
            Err(e) => {
                log::warn!(
                    "读取 Codex models_cache 备份失败，删除 Codex Cube 缓存让桌面端重新拉取: {e}"
                );
                if live_path.exists() {
                    fs::remove_file(&live_path).map_err(|err| AppError::io(&live_path, err))?;
                }
                let _ = fs::remove_file(&backup_path);
                return Ok(());
            }
        };
        if let Some(models) = restored.get_mut("models") {
            preserve_codex_models_cache_user_preferences(models, &existing);
        }
        crate::config::write_json_file(&live_path, &restored)?;
        if let Err(e) = fs::remove_file(&backup_path) {
            log::warn!("删除 Codex models_cache 备份失败: {e}");
        }
    } else if live_path.exists() {
        fs::remove_file(&live_path).map_err(|e| AppError::io(&live_path, e))?;
    }
    log::info!("[codex] models_cache.json restored/cleared after leaving provider model catalog");
    Ok(())
}
...
mod tests {
    ...
    #[test]
    fn parse_codex_version_whole_handles_pre_release_and_plain_versions() {
        assert_eq!(
            parse_codex_version_whole("codex-cli 0.147.0-alpha.6.5").as_deref(),
            Some("0.147.0")
        );
        assert_eq!(
            parse_codex_version_whole("codex-cli 0.147.0").as_deref(),
            Some("0.147.0")
        );
        assert_eq!(
            parse_codex_version_whole("codex-cli 1.2.3+build.4").as_deref(),
            Some("1.2.3")
        );
        // 非版本输出（如 --help 或报错）应返回 None
        assert_eq!(
            parse_codex_version_whole("Usage: codex [OPTIONS] [PROMPT]...").as_deref(),
            None
        );
        assert_eq!(parse_codex_version_whole("").as_deref(), None);
    }

    #[test]
    fn codex_version_json_fallback_uses_supported_version_keys() {
        assert_eq!(
            codex_version_from_version_json(&json!({ "latest_version": "0.147.0-alpha.6.5" }))
                .as_deref(),
            Some("0.147.0")
        );
        assert_eq!(
            codex_version_from_version_json(&json!({ "version": "1.2.3+build.4" })).as_deref(),
            Some("1.2.3")
        );
        assert_eq!(codex_version_from_version_json(&json!({})), None);
    }
    ...
    fn unified_session_bucket_injects_for_empty_official_config() {
        ...
        assert_eq!(
            ...
            Some(CODEX_MODEL_PROVIDER_ID)
        );
        let custom = doc["model_providers"][CODEX_MODEL_PROVIDER_ID]
            .as_table()
...
"#;
        ...
        assert_eq!(
            ...
            Some(CODEX_OFFICIAL_PROXY_PROVIDER_ID)
        );
        ...
        let provider = &doc["model_providers"][CODEX_OFFICIAL_PROXY_PROVIDER_ID];
        assert_eq!(
...
"#;
        ...
        assert!(projected_doc["model_providers"]
            .get(CODEX_OFFICIAL_PROXY_PROVIDER_ID)
            .is_some());
    ...
    fn unified_session_bucket_preserves_other_keys_and_explicit_routing() {
        let with_catalog = "model_catalog_json = \"codex-cube-model-catalog.json\"\n";
...
"#;
    ...
    #[test]
    fn codex_catalog_inherits_official_gpt_context_window_when_metadata_missing() {
        // Aggregate / relay providers often declare no per-model metadata for
        // bundled GPT slots. Without a top-level model_context_window they must
        // inherit the official Codex window (272k) instead of regressing to 128k.
        let settings = json!({
            "modelCatalog": {
                "models": [
        ...
        assert_eq!(
            window("gpt-5.6-sol"),
            Some(272_000),
            "GPT slot inherits official 272k"
        );
        assert_eq!(
            window("gpt-5.5"),
            Some(272_000),
        ...
        assert_eq!(
            ...
            Some(1_048_576),
            "explicit per-model contextWindow wins"
        );
    }
    ...
    fn codex_catalog_top_level_model_context_window_overrides_official_default() {
        ...
        let config_text = "model_context_window = 200000\n";
        let catalog = codex_model_catalog_from_settings(
            &settings,
            config_text,
            CodexCatalogToolProfile::NativeResponses,
        ...
        assert_eq!(
            catalog["models"][0]
                ...
                .and_then(Value::as_u64),
            Some(200_000),
            "explicit top-level model_context_window beats the official default"
        );
    }
    ...
    fn native_responses_profile_suppresses_apply_patch_and_keeps_shell() {
        ...
        let model_messages = entry.get("model_messages").expect(
            "native entries must carry a neutral model_messages for model-switch instructions",
        );
        assert_eq!(
            model_messages
                .get("instructions_template")
                .and_then(|v| v.as_str()),
            Some("Continue the conversation as Codex."),
            "neutral template must not carry gpt-5.5 persona text"
        );
        assert!(
            model_messages.get("instructions_variables").is_none()
                && model_messages.get("personality").is_none(),
            "neutral model_messages must not enable personality"
        );
        assert_eq!(
    ...
    #[test]
    fn aggregate_models_generate_catalog_without_model_catalog_snapshot() {
        let settings = json!({
            "aggregateModels": [
                {
                    "model": "deepseek-v4-flash@relay",
                    "providerId": "relay",
        ...
        .expect("aggregate catalog generation should not error")
        .expect("aggregateModels must yield a catalog");

        let entry = &catalog["models"][0];
        assert_eq!(
            entry.get("slug").and_then(|value| value.as_str()),
            Some("deepseek-v4-flash@relay")
        );
        assert_eq!(
            entry.get("display_name").and_then(|value| value.as_str()),
            Some("DeepSeek V4 Flash (Relay)")
        );
        assert_eq!(
            entry.get("context_window").and_then(|value| value.as_u64()),
            Some(262_144)
        );
        assert_eq!(
            entry.get("supports_parallel_tool_calls"),
    ...
    #[test]
    fn aggregate_models_count_as_inline_catalog_source_without_model_catalog() {
        let settings = json!({
            "aggregateModels": [
                { "model": "kimi-k2@relay", "providerId": "relay", "upstreamModel": "kimi-k2" }
            ]
        });

        assert!(codex_settings_has_inline_model_catalog_source(&settings));
        assert!(!codex_settings_has_inline_model_catalog_source(&json!({
            "auth": {},
            "config": ""
        })));
    }
    ...
    #[test]
    fn empty_model_catalog_is_not_a_generated_models_source() {
        // 普通供应商被注入空 modelCatalog 用于清掉旧目录指针时，绝不能把聚合
        // 槽位缓存当成可重建源而保留在 models_cache.json 里。
        let settings = json!({
            "auth": {},
            "config": "",
            "modelCatalog": { "models": [] }
        ...
        assert!(codex_settings_has_inline_model_catalog_source(&settings));
        assert!(!codex_settings_has_generated_models(&settings));

        assert!(codex_settings_has_generated_models(&json!({
...
"#;
    ...
    fn deepseek_host_native_catalog_mirrors_official_entries() {
        ...
        // apply_patch + GPT-5 harness + vendor reasoning levels). For a
    ...
    #[test]
    fn generated_deepseek_catalog_exposes_all_six_reasoning_levels() {
        let temp = tempfile::tempdir().expect("tempdir");
        let catalog_path = temp.path().join(CODEX_MODEL_CATALOG_FILENAME);
        let settings = json!({
            "modelCatalog": {
                "models": [
                    { "model": "deepseek-v4-flash", "displayName": "DeepSeek V4 Flash" },
        ...
        let prepared = prepare_codex_config_text_with_model_catalog_at(
            &settings,
            DEEPSEEK_NATIVE_CONFIG,
            CodexCatalogToolProfile::NativeResponses,
            &catalog_path,
        )
        .expect("write generated DeepSeek catalog");
        let config: toml::Value = toml::from_str(&prepared).expect("parse prepared config");
        assert_eq!(
            config
                .get("model_catalog_json")
                .and_then(toml::Value::as_str),
        ...
        let catalog: Value = serde_json::from_str(
            &fs::read_to_string(&catalog_path).expect("read generated catalog"),
        )
...
name = "any"
"#;
        let catalog_path = Path::new("/tmp/codex-cube-model-catalog.json");
        ...
        assert_eq!(
            parsed
                .get("model_catalog_json")
                .and_then(|value| value.as_str()),
            Some("/tmp/codex-cube-model-catalog.json")
        );
        assert!(
            parsed
                .get("model_providers")
...
"#;
    ...
    fn native_web_search_field_removes_own_sentinel_when_not_disabled() {
        ...
        // removing Codex Cube's own "disabled" sentinel.
...
"#;
        ...
        assert!(
            ...
            "Codex Cube's disabled sentinel should be removed when not native"
        );
    }
    ...
    fn native_web_search_field_preserves_user_value() {
        ...
        // only Codex Cube's "disabled" sentinel is owned/removable.
...
"#;
    ...
    fn resolve_catalog_path_returns_none_when_config_missing_field() {
        ...
        assert!(resolve_codex_cube_catalog_path("", &base).is_none());
        assert!(
            resolve_codex_cube_catalog_path("model = \"gpt-5\"", &base).is_none(),
            "no model_catalog_json field should yield None"
        );
    }

    #[test]
    fn resolve_catalog_path_accepts_codex_cube_owned_file() {
        let base = PathBuf::from("/tmp/.codex");
        let config = r#"model_catalog_json = "/tmp/.codex/codex-cube-model-catalog.json"
"#;
        let resolved = resolve_codex_cube_catalog_path(config, &base).expect("path resolves");
        assert_eq!(resolved, base.join(CODEX_MODEL_CATALOG_FILENAME));
    }
...
"#;
        assert!(
            resolve_codex_cube_catalog_path(config, &base).is_none(),
...
model = "glm-5"
"#;
        // Simulate a WSL UNC path as Codex Cube would see it on Windows;
        // preserve the absolute path so Desktop can resolve it directly.
        let unc_path =
            Path::new(r"\\wsl.localhost\Ubuntu\home\user\.codex\codex-cube-model-catalog.json");
        ...
        assert_eq!(written_path, unc_path.to_string_lossy());
    }

    #[test]
    fn set_catalog_json_field_writes_absolute_path_for_any_path() {
        let input = r#"model_provider = "custom"
model = "glm-5"
"#;
        let regular_path = Path::new("/home/user/.codex/codex-cube-model-catalog.json");
        ...
        assert_eq!(
            ...
            Some("/home/user/.codex/codex-cube-model-catalog.json")
        );
    }

    #[test]
    fn set_catalog_json_field_normalizes_relative_path_to_absolute() {
        let input = r#"model_provider = "custom"
model = "glm-5"
"#;
        let result = set_codex_model_catalog_json_field(
            input,
            Some(Path::new(".codex/codex-cube-model-catalog.json")),
        )
        .unwrap();
        let parsed: toml::Value = toml::from_str(&result).unwrap();
        let written = parsed
        ...
        assert!(written.ends_with(".codex/codex-cube-model-catalog.json"));
    }

    #[test]
    fn set_catalog_json_none_removes_codex_cube_owned_by_filename() {
        ...
        let input = r#"model_catalog_json = "/home/user/.codex/codex-cube-model-catalog.json"
"#;
        let result = set_codex_model_catalog_json_field(input, None).unwrap();
        let parsed: toml::Value = toml::from_str(&result).unwrap();
        assert!(
            parsed.get("model_catalog_json").is_none(),
            "None arm should remove codex-cube-owned field regardless of path format"
        );
    }
...
"#;
    ...
    fn set_catalog_json_some_preserves_user_owned_catalog() {
        // When Codex-Cube generates a catalog (Some arm), it must still respect a
        // user-managed external catalog file instead of clobbering it with the
        // codex-cube-owned filename. Only an absent or codex-cube-owned pointer is
...
"#;
    ...
    #[test]
    fn scrub_codex_provider_active_removes_section_and_routing_fields() {
        let input = r#"model_provider = "custom"
model = "gpt-5.5"
...
[desktop]
app_server = { url = "http://localhost:15721" }
"#;

        let result =
            scrub_codex_provider_from_config(input, "custom", Some("https://api.example.com/v1/"))
                .unwrap();
        let parsed: toml::Value = toml::from_str(&result).unwrap();
        assert!(
            parsed.get("model_provider").is_none(),
            "model_provider must be removed"
        );
        assert!(parsed.get("model").is_none(), "model must be removed");
        assert!(
            parsed.get("base_url").is_none(),
            "matching top-level base_url must be removed"
        );
        assert!(
            parsed.get("model_providers").is_none(),
            "empty model_providers table must be removed"
        );
        assert!(parsed.get("plugins").is_some(), "plugins must be preserved");
        assert!(
            parsed.get("mcp_servers").is_some(),
            "mcp_servers must be preserved"
        );
        assert!(parsed.get("desktop").is_some(), "desktop must be preserved");
    }

    #[test]
    fn scrub_codex_provider_inactive_leaves_config_unchanged() {
        let input = r#"model_provider = "other"
model = "gpt-5.5"
...
"#;
        ...
        let result =
            scrub_codex_provider_from_config(input, "custom", Some("https://api.example.com/v1"))
                .unwrap();
        assert_eq!(result, input, "non-active provider scrub must be a no-op");
    }

    #[test]
    fn scrub_codex_provider_removes_inline_table_section() {
        let input = r#"model_provider = "custom"
model_providers = { custom = { name = "custom" }, other = { name = "other" } }
"#;
        let result = scrub_codex_provider_from_config(input, "custom", None).unwrap();
        let parsed: toml::Value = toml::from_str(&result).unwrap();
        assert!(
            parsed.get("model_provider").is_none(),
            "model_provider must be removed"
        ...
        assert!(
            parsed
                ...
                .and_then(|v| v.get("custom"))
                .is_none(),
            "inline-table provider section must be removed"
        );
        assert!(
            parsed
                .get("model_providers")
                ...
                .is_some(),
            "remaining inline provider entries must be preserved"
        );
    }

    #[test]
    fn scrub_codex_provider_removes_empty_model_providers_table() {
        let input = r#"model_provider = "custom"
...
"#;
        ...
        assert!(
            parsed.get("model_providers").is_none(),
            "whole model_providers key must be removed once empty"
        );
        assert!(parsed.get("model_provider").is_none());
    }

    #[test]
    fn scrub_codex_provider_removes_catalog_pointer_only_when_active() {
        let active = r#"model_provider = "custom"
model = "gpt-5.5"
model_catalog_json = "/home/user/.codex/codex-cube-model-catalog.json"

[model_providers.custom]
name = "custom"
"#;
        let result = scrub_codex_provider_from_config(active, "custom", None).unwrap();
        let parsed: toml::Value = toml::from_str(&result).unwrap();
        assert!(
            parsed.get("model_catalog_json").is_none(),
            "codex-cube catalog pointer must be removed for the active provider"
        );

        let inactive = r#"model_provider = "other"
model_catalog_json = "/home/user/.codex/codex-cube-model-catalog.json"
"#;
        let result = scrub_codex_provider_from_config(inactive, "custom", None).unwrap();
        let parsed: toml::Value = toml::from_str(&result).unwrap();
        assert_eq!(
            parsed.get("model_catalog_json").and_then(|v| v.as_str()),
            Some("/home/user/.codex/codex-cube-model-catalog.json"),
            "catalog pointer must survive when the provider is not active"
        );

        let user_owned = r#"model_provider = "custom"
model_catalog_json = "/home/user/.codex/my-custom-catalog.json"

[model_providers.custom]
name = "custom"
"#;
        let result = scrub_codex_provider_from_config(user_owned, "custom", None).unwrap();
        let parsed: toml::Value = toml::from_str(&result).unwrap();
        assert_eq!(
            parsed.get("model_catalog_json").and_then(|v| v.as_str()),
            Some("/home/user/.codex/my-custom-catalog.json"),
            "user-owned catalog pointer must be preserved"
        );
    }
...
"#;
        ...
        let result = resolve_codex_cube_catalog_path(config_text, &base_dir);
        assert_eq!(
            result,
            Some(base_dir.join(CODEX_MODEL_CATALOG_FILENAME)),
    ...
    fn resolve_catalog_rejects_absolute_path_outside_config_dir() {
        let config_text = r#"model_catalog_json = "/tmp/secret/codex-cube-model-catalog.json"
"#;
        let base_dir = PathBuf::from("/home/user/.codex");
        let result = resolve_codex_cube_catalog_path(config_text, &base_dir);
        assert_eq!(
    ...
    fn resolve_catalog_accepts_absolute_path_inside_config_dir() {
        let config_text = r#"model_catalog_json = "/home/user/.codex/codex-cube-model-catalog.json"
"#;
        let base_dir = PathBuf::from("/home/user/.codex");
        let result = resolve_codex_cube_catalog_path(config_text, &base_dir);
        assert_eq!(
    ...
    fn resolve_catalog_rejects_traversal_to_parent_directory() {
        let config_text = r#"model_catalog_json = "../codex-cube-model-catalog.json"
"#;
        let base_dir = PathBuf::from("/home/user/.codex");
        let result = resolve_codex_cube_catalog_path(config_text, &base_dir);
        assert_eq!(
    ...
    fn resolve_catalog_rejects_symlink_escaping_config_dir() {
        ...
        // "link/codex-cube-model-catalog.json" 词法上在 base 内，真实读取却落到
        ...
        let escaped_file = outside_dir.join(CODEX_MODEL_CATALOG_FILENAME);
        ...
        let config_text = r#"model_catalog_json = "link/codex-cube-model-catalog.json"
"#;
        let result = resolve_codex_cube_catalog_path(config_text, &base_dir);
        assert_eq!(
            result, None,
    ...
    fn resolve_catalog_accepts_real_file_inside_config_dir() {
        ...
        let base_dir = temp.path().join("codex");
        fs::create_dir_all(&base_dir).expect("create base");
        let catalog_file = base_dir.join(CODEX_MODEL_CATALOG_FILENAME);
...
"#;
    ...
    #[test]
    fn aggregate_build_takeover_toml_routes_through_proxy() {
        let provider = crate::provider::Provider::with_id(
            ...
            json!({
                "aggregateModels": [
                    ...
                    { "model": "deepseek-chat", "providerId": "ds", "upstreamModel": "deepseek-chat" }
                ]
            }),
            None,
        );
        let toml_text = build_aggregate_takeover_toml(&provider, "http://127.0.0.1:15721/v1", "")
        ...
        assert_eq!(
            parsed["model"].as_str(),
            Some("gpt-5.5@neko"),
            "default model should be the first aggregate slot"
        );
        let custom = &parsed["model_providers"]["custom"];
        assert_eq!(custom["name"].as_str(), Some("My Aggregate"));
        assert_eq!(
    ...
    #[test]
    fn aggregate_build_takeover_toml_preserves_existing_user_settings() {
        let provider = crate::provider::Provider::with_id(
            "agg-1".to_string(),
            "My Aggregate".to_string(),
            json!({
...
"#;
        ...
        assert!(
            ...
            "stale proxy token from the previous takeover must be replaced"
        );
        assert!(
            parsed.get("base_url").is_none(),
            "aggregate config must not keep a stale top-level base_url"
        );
        assert_eq!(
            parsed["desktop"]["localeOverride"].as_str(),
            Some("en-US"),
            "user Codex language setting must be preserved"
        );
        assert_eq!(
            parsed["plugins"]["chrome@openai-bundled"]["enabled"].as_bool(),
            Some(true),
            "user plugin settings must be preserved"
        );
        assert_eq!(
            parsed["mcp_servers"]["node_repl"]["command"].as_str(),
            Some("/bin/true"),
            "user MCP server settings must be preserved"
        );
    }
        ...
        let provider = crate::provider::Provider::with_id(
            ...
            json!({
                "aggregateModels": [
                    ...
                    { "model": "gpt-5.6-terra", "providerId": "opencode", "upstreamModel": "deepseek-v4-flash" }
                ]
            }),
            None,
        );
...
"#;
    ...
    #[test]
    fn aggregate_build_takeover_toml_prefers_explicit_default_model() {
        let provider = crate::provider::Provider::with_id(
            "agg-1".to_string(),
            "My Aggregate".to_string(),
            json!({
        ...
        // 用户显式选择的默认模型优先，即使现有 config.toml 已有合法聚合模型。
        ...
        assert_eq!(
            parsed["model"].as_str(),
            Some("deepseek-chat"),
            "wizard-selected default model must win"
        );
    }

    #[test]
    fn aggregate_build_takeover_toml_ignores_invalid_explicit_default_model() {
        let provider = crate::provider::Provider::with_id(
            "agg-1".to_string(),
            "My Aggregate".to_string(),
            json!({
                "defaultModel": "not-in-aggregate-list",
                "aggregateModels": [
                    { "model": "gpt-5.5@neko", "providerId": "neko", "upstreamModel": "gpt-5.5" },
                    { "model": "deepseek-chat", "providerId": "ds", "upstreamModel": "deepseek-chat" }
                ]
    ...
    #[test]
    fn merge_codex_live_user_settings_into_backup_preserves_preferences() {
        let backup = json!({
        ...
        assert!(
            ...
            "reasoning effort must survive restore"
        );
        assert!(
            cfg.contains("personality = \"pragmatic\""),
            "personality must survive restore"
        );
        assert!(
            cfg.contains("disable_response_storage = true"),
            "disable_response_storage must survive restore"
        );
        assert!(
            cfg.contains("model_reasoning_summary = \"concise\""),
            "reasoning summary must survive restore"
        );
        assert!(
            cfg.contains("model_verbosity = \"concise\""),
            "verbosity must survive restore"
        );
        assert!(
            cfg.contains("localeOverride = \"en-US\""),
            "desktop settings must survive restore"
        );
        // 接管投影字段保留备份的原始值，不残留本地代理
        assert!(
            cfg.contains("model_provider = \"deepseek\""),
            "original model_provider must survive"
        );
        assert!(
            cfg.contains("https://api.deepseek.com/v1"),
            "original provider base_url must survive"
        );
        assert!(
            !cfg.contains("127.0.0.1"),
            "proxy base_url must not leak into restored config"
        );
        assert!(
            !cfg.contains("PROXY_MANAGED"),
            "proxy placeholder must not leak into restored config"
        );
        assert!(
            cfg.contains("model = \"gpt-5.4\""),
            "backup's own model must survive restore"
        );
        assert!(
            !cfg.contains("gpt-5.6-sol"),
            "live aggregate slot model must not leak into restored config"
        );
        assert_eq!(
            merged
                .get("auth")
                ...
                .and_then(Value::as_str),
            Some("sk-original"),
            "original auth must survive"
        );
    }

    #[test]
    fn merge_codex_live_permission_settings_preserves_access_without_route_leakage() {
        let target = json!({
            "auth": { "OPENAI_API_KEY": "sk-target" },
            "config": r#"model_provider = "opencode"
model = "deepseek-v4-flash"
...
"#
        ...
        let merged = merge_codex_live_permission_settings(&target, &live);
        let config = merged["config"].as_str().expect("merged config");
        ...
        assert!(
            parsed.get("sandbox_mode").is_none(),
            "legacy sandbox_mode must not conflict with default_permissions"
        );
        assert_eq!(parsed["approval_policy"].as_str(), Some("never"));
        assert_eq!(
            parsed["projects"]["/Users/test/project"]["trust_level"].as_str(),
            Some("trusted")
        );
        assert_eq!(parsed["model"].as_str(), Some("deepseek-v4-flash"));
        assert_eq!(parsed["model_provider"].as_str(), Some("opencode"));
        assert_eq!(
            parsed["model_providers"]["opencode"]["base_url"].as_str(),
            Some("https://opencode.ai/zen/go/v1")
        );
        assert!(
            !config.contains("gpt-5.4") && !config.contains("127.0.0.1"),
            "aggregate routing fields must not leak into the target provider: {config}"
        );
    }

    #[test]
    fn merge_codex_live_permission_settings_keeps_target_when_live_has_no_access_keys() {
        let target = json!({
        ...
        assert_eq!(
            merge_codex_live_permission_settings(&target, &live),
            target,
            "a permission-less live config must not erase the provider snapshot"
        );
    }

    #[test]
    fn merge_codex_live_permission_settings_supports_configless_official_provider() {
        let target = json!({ "auth": {} });
        let live = json!({
        ...
        let parsed: toml::Value =
            toml::from_str(merged["config"].as_str().expect("generated config"))
                .expect("valid TOML");
        assert_eq!(
            parsed["default_permissions"].as_str(),
            Some(":danger-full-access")
    ...
    #[test]
    fn merge_codex_live_user_settings_ignores_non_takeover_live() {
        let backup = json!({
            "auth": {},
            "config": "model_provider = \"deepseek\"\nmodel = \"gpt-5.4\"\n"
        });
        // Live 没有代理投影（例如用户手写配置），不应污染备份
        let live = json!({
            "auth": {},
            "config": "model_provider = \"deepseek\"\nmodel = \"gpt-5.4\"\nmodel_reasoning_effort = \"max\"\n"
        });
        ...
        assert_eq!(
            ...
            backup.get("config").and_then(Value::as_str),
            "non-takeover live config must not be merged"
        );
    }

    #[test]
    fn aggregate_model_entries_parse_camel_case() {
        let settings = json!({
            "aggregateModels": [
                { "model": "deepseek-chat", "providerId": "deepseek", "upstreamModel": "deepseek-chat" },
                { "model": "deepseek-chat@kimi", "providerId": "kimi", "upstreamModel": "deepseek-chat" },
                { "model": "  ", "providerId": "bad" },
                { "model": "no-provider" },
                { "model": "kimi-k2", "providerId": "kimi" }
            ]
        });
        let entries = codex_aggregate_model_entries(&settings);
        assert_eq!(entries.len(), 3);
        assert_eq!(entries[0].model, "deepseek-chat");
        assert_eq!(entries[0].provider_id, "deepseek");
        assert_eq!(entries[0].upstream_model.as_deref(), Some("deepseek-chat"));
        assert_eq!(entries[1].model, "deepseek-chat@kimi");
        assert_eq!(entries[1].provider_id, "kimi");
        assert_eq!(entries[2].upstream_model, None);
    }

    #[test]
    fn aggregate_model_entries_parse_snake_case() {
        let settings = json!({
            "aggregateModels": [
                { "model": "glm-5", "provider_id": "zhipu", "upstream_model": "glm-5" }
            ]
        });
        let entries = codex_aggregate_model_entries(&settings);
        assert_eq!(entries.len(), 1);
        assert_eq!(entries[0].model, "glm-5");
        assert_eq!(entries[0].provider_id, "zhipu");
        assert_eq!(entries[0].upstream_model.as_deref(), Some("glm-5"));
    }

    #[test]
    fn aggregate_models_cache_write_backs_up_and_restores_original() {
        let temp = tempfile::tempdir().expect("tempdir");
        let codex_dir = temp.path().join("codex");
        fs::create_dir_all(&codex_dir).expect("create codex dir");
        let live_path = codex_dir.join("models_cache.json");
        let original = json!({
            "models": [{ "slug": "gpt-5.5", "display_name": "GPT-5.5", "context_window": 128000 }]
        });
        fs::write(
            &live_path,
        ...
        write_codex_models_cache_for_provider_at(&codex_dir, &settings, "", Some("0.147.0"))
            .expect("write aggregate cache");

        let written: Value =
            serde_json::from_str(&fs::read_to_string(&live_path).expect("read cache"))
                .expect("parse cache");
        assert!(
            written
                .get("etag")
                .and_then(Value::as_str)
                .is_some_and(|etag| etag.starts_with("W/\"codex-cube-aggregate-")),
            "aggregate cache must carry codex-cube etag"
        );
        assert_eq!(
            written.get("client_version").and_then(Value::as_str),
            Some("0.147.0"),
            "aggregate cache must carry the codex client_version or Codex rejects it"
        );
        let slugs: Vec<&str> = written
            ...
            .unwrap_or_default();
        assert_eq!(slugs, vec!["deepseek-chat", "kimi-k2"]);

        let first_model = &written["models"][0];
        let efforts: Vec<&str> = first_model["supported_reasoning_levels"]
            .as_array()
            .expect("supported reasoning levels")
            .iter()
            .filter_map(|level| level.get("effort").and_then(Value::as_str))
        ...
        assert_eq!(
            efforts,
            vec!["low", "medium", "high", "xhigh", "max", "ultra"],
            "Desktop cache must use Desktop-supported reasoning levels"
        );
        assert_eq!(
            first_model
                .get("default_reasoning_level")
                .and_then(Value::as_str),
            Some("high"),
            "Desktop cache default must be a Desktop-supported effort"
        );

        // 备份必须存在，恢复后回到原始缓存
        restore_or_clear_codex_models_cache(&codex_dir).expect("restore cache");
        let restored: Value =
            serde_json::from_str(&fs::read_to_string(&live_path).expect("read cache"))
        ...
        assert_eq!(
            restored
                ...
                .map(Vec::len),
            Some(1),
            "restore must bring back the original cache"
        );
        assert!(
            restored
                .get("etag")
                .and_then(Value::as_str)
                .is_none_or(|etag| !etag.starts_with("W/\"codex-cube-")),
            "restored cache must not be codex-cube-owned"
        );
    }

    #[test]
    fn aggregate_models_cache_preserves_user_preferences_and_stable_etag() {
        let temp = tempfile::tempdir().expect("tempdir");
        let codex_dir = temp.path().join("codex");
        fs::create_dir_all(&codex_dir).expect("create codex dir");
        let live_path = codex_dir.join("models_cache.json");
        ...
        let settings = json!({
            "modelCatalog": {
                "models": [
                    { "model": "gpt-5.4", "displayName": "DeepSeek V4 Flash", "contextWindow": 1048576 }
                ]
            }
        });
        write_codex_models_cache_for_provider_at(&codex_dir, &settings, "", Some("0.147.0"))
            .expect("write initial aggregate cache");
        let first: Value =
            serde_json::from_str(&fs::read_to_string(&live_path).expect("read cache"))
                .expect("parse cache");
        ...
        // 模拟用户在桌面端把 gpt-5.4 的推理档位调到 max，并设置语气
        let mut edited = first;
        edited["models"][0]["default_reasoning_level"] = json!("max");
        edited["models"][0]["default_reasoning_summary"] = json!("concise");
        edited["models"][0]["default_tone"] = json!("casual");
        fs::write(
            &live_path,
            serde_json::to_string(&edited).expect("serialize"),
        )
        .expect("write edited cache");

        // 再次接管重建：用户偏好必须保留，且模型列表未变时 etag 保持稳定
        write_codex_models_cache_for_provider_at(&codex_dir, &settings, "", Some("0.147.0"))
            .expect("rewrite aggregate cache");
        let rewritten: Value =
            serde_json::from_str(&fs::read_to_string(&live_path).expect("read cache"))
                .expect("parse cache");
        let model = &rewritten["models"][0];
        assert_eq!(
            model.get("default_reasoning_level").and_then(Value::as_str),
            Some("max"),
            "user reasoning effort must survive cache regeneration"
        );
        assert_eq!(
            model
                ...
                .and_then(Value::as_str),
            Some("concise"),
            "user reasoning summary must survive cache regeneration"
        );
        assert_eq!(
            model.get("default_tone").and_then(Value::as_str),
            Some("casual"),
            "user default tone must survive cache regeneration"
        );
        assert_eq!(
            rewritten.get("etag").and_then(Value::as_str),
            Some(first_etag.as_str()),
            "etag must stay stable when the model list and preferences are unchanged"
        );
    }

    #[test]
    fn aggregate_models_cache_restore_preserves_user_preferences() {
        let temp = tempfile::tempdir().expect("tempdir");
        let codex_dir = temp.path().join("codex");
        fs::create_dir_all(&codex_dir).expect("create codex dir");
        let live_path = codex_dir.join("models_cache.json");

        // 预置一份原始缓存（非 Codex Cube 拥有），首次接管会把它备份起来
        fs::write(
            &live_path,
            serde_json::to_string(&json!({
                "client_version": "0.147.0",
                "etag": "W/\"original\"",
                "models": [{ "slug": "gpt-5.4", "display_name": "DeepSeek V4 Flash" }]
            }))
            .expect("serialize original cache"),
        )
        .expect("write original cache");
        ...
        // 模拟用户在桌面端把推理档位调到 max 并设置语气
        let mut edited: Value =
            serde_json::from_str(&fs::read_to_string(&live_path).expect("read cache"))
                .expect("parse cache");
        edited["models"][0]["default_reasoning_level"] = json!("max");
        edited["models"][0]["default_tone"] = json!("friendly");
        fs::write(
            &live_path,
            serde_json::to_string(&edited).expect("serialize"),
        )
        ...
        // 离开 Provider / 关闭接管：恢复原缓存时用户偏好必须保留
        restore_or_clear_codex_models_cache(&codex_dir).expect("restore cache");
        let restored: Value =
            serde_json::from_str(&fs::read_to_string(&live_path).expect("read restored cache"))
                .expect("parse restored cache");
        assert_eq!(
            restored["models"][0]
                .get("default_reasoning_level")
                .and_then(Value::as_str),
            Some("max"),
            "user reasoning effort must survive models_cache restore"
        );
        assert_eq!(
            restored["models"][0]
                .get("default_tone")
                .and_then(Value::as_str),
            Some("friendly"),
            "user tone must survive models_cache restore"
        );
    }

    #[test]
    fn aggregate_models_cache_clears_when_no_backup() {
        let temp = tempfile::tempdir().expect("tempdir");
        let codex_dir = temp.path().join("codex");
        fs::create_dir_all(&codex_dir).expect("create codex dir");
        let live_path = codex_dir.join("models_cache.json");
        ...
        // 无原始缓存（首次写入时没有备份）→ 清除 Codex Cube 缓存
        restore_or_clear_codex_models_cache(&codex_dir).expect("clear cache");
        assert!(
            !live_path.exists(),
            "codex-cube-owned cache must be removed"
        );
    }

    #[test]
    fn aggregate_models_cache_restore_is_idempotent_for_user_cache() {
        let temp = tempfile::tempdir().expect("tempdir");
        let codex_dir = temp.path().join("codex");
        fs::create_dir_all(&codex_dir).expect("create codex dir");
        let live_path = codex_dir.join("models_cache.json");
        let user_cache = json!({ "models": [{ "slug": "gpt-5.5" }] });
        fs::write(
            &live_path,
            serde_json::to_string(&user_cache).expect("serialize"),
        )
        .expect("write user cache");

        // 非 Codex Cube 缓存：restore 必须保持原样
        restore_or_clear_codex_models_cache(&codex_dir).expect("restore no-op");
        let after: Value =
            serde_json::from_str(&fs::read_to_string(&live_path).expect("read cache"))
                .expect("parse cache");
        assert_eq!(
            after.get("models").and_then(Value::as_array).map(Vec::len),
            Some(1)
        );
    }
}
