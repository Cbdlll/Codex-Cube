...
//!
//! Handles batch import of MCP server configurations via codexcube:// URLs.
...
pub(crate) fn parse_mcp_apps(apps_str: &str) -> Result<McpApps, AppError> {
    let mut apps = McpApps { codex: false };
...
mod tests {
    ...
    #[test]
    fn enabled_apps_merge_covers_supported_mcp_client() {
        let existing = McpApps { codex: false };
        let target = McpApps { codex: true };
