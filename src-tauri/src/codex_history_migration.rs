...
use crate::codex_config::{get_codex_config_dir, read_codex_config_text, CODEX_MODEL_PROVIDER_ID};
    ...
    if source_provider_ids.is_empty() {
        crate::settings::mark_codex_third_party_history_provider_bucket_migrated(
            CodexThirdPartyHistoryProviderBucketMigration {
                ...
                target_provider_id: CODEX_MODEL_PROVIDER_ID.to_string(),
...
/// 迁移前 jsonl / state DB 均备份到 `~/.codex-cube/backups/codex-official-history-unify-v1/`。
    ...
    config_text
        ...
        .and_then(|doc| {
            doc.get("model_provider")
                ...
                .map(|id| id.trim() == CODEX_MODEL_PROVIDER_ID)
        })
        .unwrap_or(false)
}
...
fn rewrite_codex_session_meta_line_for_restore(
    ...
    if payload.get("model_provider")?.as_str()? != CODEX_MODEL_PROVIDER_ID {
        return None;
    }
...
fn restore_codex_state_db_official_threads(
    ...
    for chunk in ids.chunks(STATE_DB_ID_CHUNK) {
        ...
        values.push(CODEX_MODEL_PROVIDER_ID.to_string());
...
fn migrate_provider_config_template_to_custom(
    ...
    let custom_table_exists = config_defines_model_provider(&doc, CODEX_MODEL_PROVIDER_ID);
    ...
    if let Some(source_provider_id) = source_provider_id_to_move {
        ...
        model_providers[CODEX_MODEL_PROVIDER_ID] = provider_table;
        changed = true;
    }
...
fn rewrite_legacy_provider_profile_refs(doc: &mut DocumentMut, source_provider_id: &str) -> bool {
    ...
    for profile_key in profile_keys {
        ...
        if references_legacy {
            profile_table.insert("model_provider", toml_edit::value(CODEX_MODEL_PROVIDER_ID));
            changed = true;
        }
    }
    changed
...
fn rewrite_codex_session_meta_line(
    ...
    payload.insert(
        "model_provider".to_string(),
        Value::String(CODEX_MODEL_PROVIDER_ID.to_string()),
    );
    serde_json::to_string(&value).ok()
}
...
"#
    ...
    #[test]
    fn skips_custom_category_unknown_provider_when_created_by_codex_cube() {
        let db = Database::memory().expect("memory db");
        let mut provider = Provider::with_id(
            "generated-uuid".to_string(),
            "Manual Relay".to_string(),
