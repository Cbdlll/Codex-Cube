...
/// 1M 上下文窗口的模型名标记（如 `claude-sonnet-4-6[1m]`）。
...
/// Models that Codex-Cube is willing to advertise to clients as text-only.
///
...
fn normalize_model_id(value: &str) -> String {
    let mut normalized = value
        ...
        .trim()
        .to_ascii_lowercase();
    if let Some(stripped) = normalized.strip_suffix(crate::model_capabilities::ONE_M_CONTEXT_MARKER)
    {
