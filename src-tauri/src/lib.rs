...
pub fn run() {
    // 设置 panic hook，在应用崩溃时记录日志到 <app_config_dir>/crash.log（默认 ~/.codex-cube/crash.log）
    ...
    let builder = builder
        ...
        .setup(|app| {
            ...
            // 一次性迁移旧版配置目录 ~/.codex-cube -> ~/.codex-cube（仅新目录不存在时）
            ...
            // 初始化日志（输出到 <app_config_dir>/logs/codex-cube.log）
            {
                    ...
                    tauri_plugin_log::Builder::default()
                        ...
                        .targets([
                            ...
                            Target::new(TargetKind::Folder {
                                path: log_dir,
                                file_name: Some("codex-cube".into()),
                            }),
                        ])
            ...
            log::info!("=== Codex-Cube v{} started ===", env!("CARGO_PKG_VERSION"));
            }
            ...
            let db_path = app_config_dir.join("codex-cube.db");
            ...
            //
            // 捕获首次运行快照：所有全新装用户都会看到欢迎弹窗介绍 Codex-Cube 的工作方式。
            ...
            #[cfg(any(target_os = "linux", all(debug_assertions, windows)))]
            {
                #[cfg(target_os = "linux")]
                {
                    // tauri-plugin-deep-link 以「当前可执行文件名-handler.desktop」命名，
                    // 与插件保持一致（不同构建的二进制名不同，不能写死旧名）。
                    ...
                    let handler_name = std::env::current_exe()
                        ...
                        .and_then(|exe| {
                            exe.file_name().map(|name| {
                                format!("{}-handler.desktop", name.to_string_lossy())
                            })
                        })
                        .unwrap_or_default();
                    let should_register = app
                        .path()
                        .data_dir()
                        .map(|d| !d.join("applications").join(&handler_name).exists())
                        .unwrap_or(true);
                ...
                move |event| {
                    ...
                    for (i, url) in urls.iter().enumerate() {
                        ...
                        if handle_deeplink_url(&app_handle, url_str, true, "on_open_url") {
                            break; // Process only first deep link URL
                        }
                    }
                }
            });
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            commands::list_codex_subagents,
            commands::upsert_codex_subagent,
            commands::delete_codex_subagent,
            commands::fetch_codex_subagent_models,
...
fn show_migration_error_dialog(app: &tauri::AppHandle, error: &str) -> bool {
    ...
    let message = if is_chinese_locale() {
        ...
        format!(
            ...
            Consider rolling back to an older Codex-Cube version.\n\n\
...
fn show_database_init_error_dialog(
    ...
    let message = if is_chinese_locale() {
        ...
        format!(
            ...
            Suggestions:\n\
            1) Back up the entire config directory (including codex-cube.db)\n\
            2) If you see “database version is newer”, please upgrade Codex-Cube\n\
