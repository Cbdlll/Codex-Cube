...
///   且不一定等于用户目录，可能导致 `.codex-cube/codex-cube.db` 路径变化，从而“看起来像数据丢失”。
///
/// ## 测试隔离
///
/// 为了让 Windows CI/本地测试能稳定隔离真实用户数据，可通过 `CODEX_CUBE_TEST_HOME`
/// 显式覆盖 home dir（仅用于测试/调试场景）。
pub fn get_home_dir() -> PathBuf {
    if let Ok(home) = std::env::var("CODEX_CUBE_TEST_HOME") {
...
/// re-verify containment (see `resolve_codex_cube_catalog_path`).
...
fn default_mcp_path_for_config_dir(dir: &Path) -> Option<PathBuf> {
    derive_wsl_default_mcp_path(dir)
}
...
/// 获取应用配置目录路径 (~/.codex-cube)
pub fn get_app_config_dir() -> PathBuf {
    if let Some(custom) = crate::app_store::get_app_config_dir_override() {
        return custom;
    }

    let default_dir = get_home_dir().join(".codex-cube");

    // 兼容 v3.10.3：当用户环境存在 `HOME` 且与真实用户目录不同，
    // v3.10.3 可能在 `HOME/.codex-cube/` 下创建/使用了数据库。
    // 这里仅在“默认位置没有数据库”时回退到旧位置，避免再次出现“供应商消失”问题，
    // 同时也避免新安装因为 `HOME` 被设置而写入非预期路径。
    #[cfg(windows)]
    {
        let default_db = default_dir.join("codex-cube.db");
        if !default_db.exists() {
            if let Ok(home_env) = std::env::var("HOME") {
                let trimmed = home_env.trim();
                if !trimmed.is_empty() {
                    let legacy_dir = PathBuf::from(trimmed).join(".codex-cube");
                    if legacy_dir.join("codex-cube.db").exists() {
                        log::info!(
...
/// 一次性迁移旧版配置目录 `~/.codex-cube` → `~/.codex-cube`。
/// 仅当新目录不存在且旧目录存在时执行（整目录重命名，保留全部数据）。
pub fn migrate_legacy_config_dir() {
    let home = get_home_dir();
    let old = home.join(".codex-cube");
    let new = home.join(".codex-cube");
...
mod tests {
    ...
    #[cfg(windows)]
    #[test]
    #[ignore = "requires CODEX_CUBE_WSL_TEST_DIR to point to a WSL2 UNC directory"]
    fn atomic_write_replaces_existing_wsl_unc_file() {
        let root = PathBuf::from(
            std::env::var_os("CODEX_CUBE_WSL_TEST_DIR")
                .expect("CODEX_CUBE_WSL_TEST_DIR must be set"),
        );
        let home = get_home_dir();
